unit IntegrateGPInsuranceTaxiU;
...
type
  TIntegrateGPInsuranceTaxi = class(TForm)
...
var
  IntegrateGPInsuranceTaxi: TIntegrateGPInsuranceTaxi;
...
procedure TIntegrateGPInsuranceTaxi.Init;
begin
  FromDates.Date:=Now();
  Lokasi.ItemIndex:=-1;
  LokasiDisp.Text:='';
...
procedure TIntegrateGPInsuranceTaxi.InitGrid1;
...
procedure TIntegrateGPInsuranceTaxi.InitGrid2;
var IntCount,IntCount2:Integer;
begin
  for IntCount:=0 to StrGrid2.RowCount-1 do
    for IntCount2:=0 to StrGrid2.ColCount-1 do
...
procedure TIntegrateGPInsuranceTaxi.RefreshJenis;
var StrQry:String;
    Qry:TADOQuery;
    IntCount:Integer;
begin
...
procedure TIntegrateGPInsuranceTaxi.RefreshCombo;
var QStr:String;
    Qry:TADOQuery;
    Count:Integer;
begin
...
procedure TIntegrateGPInsuranceTaxi.RefreshData1;
...
begin
  ...
  if Main.OpenDb then begin
    ...
    StrQry:='EXEC IntegrateGPTX5 '+QuotedStr(FormatDateTime('yyyy/mm/dd',FromDates.Date))+','+QuotedStr(Lokasi.Text)+',@Total=1'+';';
    Qry.SQL.Add(StrQry);
    Qry.Open;
    ...
    Qry.Close;
    StrQry:='EXEC IntegrateGPTX6 '+QuotedStr(FormatDateTime('yyyy/mm/dd',FromDates.Date))+','+QuotedStr(Lokasi.Text)+';';
    Qry.SQL.Clear;
    Qry.SQL.Add(StrQry);
    Qry.Open;
    IntCount:=0;
...
procedure TIntegrateGPInsuranceTaxi.RefreshData2;
var Qry,Qry2:TADOQuery;
...
begin
  ...
  if Main.OpenDb then begin
    ...
    IntCount:=1;
    StrQry:='EXEC IntegrateGPTX5 '+QuotedStr(FormatDateTime('yyyy/mm/dd',FromDates.Date))+','+QuotedStr(Lokasi.Text)+',@Total=0'+';';    Qry.SQL.Add(StrQry);
    Qry.Open;
    if Qry.RecordCount>0 then begin
      if Qry.FieldValues['total']<>NULL then begin
    ...
    StrLastIndex1:='';
    StrLastIndex2:='';
    StrQry:='EXEC IntegrateGPTX7 '+QuotedStr(FormatDateTime('yyyy/mm/dd',FromDates.Date))+','+QuotedStr(Lokasi.Text)+';';
    Qry.SQL.Clear;
    Qry.SQL.Add(StrQry);
    Qry.Open;
    if Qry.RecordCount>0 then while not(Qry.Eof) do begin
    ...
    Qry.Close;
    Inc(IntCount);
    StrQry:='EXEC IntegrateGPTX8 '+QuotedStr(FormatDateTime('yyyy/mm/dd',FromDates.Date))+','+QuotedStr(Lokasi.Text)+';';
    Qry.SQL.Clear;
    Qry.SQL.Add(StrQry);
    Qry.Open;
    IntCount2:=1;
...
procedure TIntegrateGPInsuranceTaxi.FormShow(Sender: TObject);
begin
  Init;
  RefreshCombo;
  RefreshJenis;
...
procedure TIntegrateGPInsuranceTaxi.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TIntegrateGPInsuranceTaxi.SelesaiClick(Sender: TObject);
begin
  Close;
end;

procedure TIntegrateGPInsuranceTaxi.ToXCel1Click(Sender: TObject);
begin
  if ToExcel4(StrGrid1) then ShowMessage('Export ke Excel Berhasil')
  else ShowMessage('Export ke Excel Gagal');
end;

procedure TIntegrateGPInsuranceTaxi.ToXcel2Click(Sender: TObject);
begin
...
procedure TIntegrateGPInsuranceTaxi.ProcessClick(Sender: TObject);
begin
  ProcessData;
end;

procedure TIntegrateGPInsuranceTaxi.ProcessData;
begin
  InitGrid1;
  InitGrid2;
  RefreshData1;
...
procedure TIntegrateGPInsuranceTaxi.GPLastGLKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Not(Key In ['0'..'9',#8,#13]) then Key:=#0;
end;

procedure TIntegrateGPInsuranceTaxi.LokasiChange(Sender: TObject);
begin
  LokasiDisp.Text:=LokasiArr[Lokasi.ItemIndex][1];  
end;
