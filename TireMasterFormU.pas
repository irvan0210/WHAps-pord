...
var
  TireMasterForm: TTireMasterForm;
  StrMenuId:String;
  IsView:Boolean;
...
constructor TTireMasterForm.Create(AOwner:TComponent;MenuId:String;IsViewOnly:Boolean=False);
begin
  StrMenuId:=MenuId;
  IsView:=IsViewOnly;
...
procedure TTireMasterForm.Init;
begin
...
procedure TTireMasterForm.LoadData;
var Qry:TADOQuery;
    StrQry:String;
begin
...
procedure TTireMasterForm.Input(IsEnable:Boolean);
begin
  GroupInput.Enabled:=IsEnable;
  Simpan.Visible:=IsEnable; 
end;

procedure TTireMasterForm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TTireMasterForm.SelesaiClick(Sender: TObject);
begin
  TireMasterForm.Close;
end;

procedure TTireMasterForm.FormShow(Sender: TObject);
begin
  Init;
...
procedure TTireMasterForm.SimpanClick(Sender: TObject);
var Qry:TADOQuery;
...
begin
  ...
  if IsOk then TireMasterForm.Close;
end;

procedure TTireMasterForm.MenuNameKeyPress(Sender: TObject; var Key: Char);
begin
  if Key=#13 then Description.SetFocus;
end;

procedure TTireMasterForm.DescriptionKeyPress(Sender: TObject; var Key: Char);
begin
  if Key=#13 then Active.SetFocus;
end;

procedure TTireMasterForm.ActiveKeyPress(Sender: TObject; var Key: Char);
begin
