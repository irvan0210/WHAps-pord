unit VhcOutTimeGrossMonthlyRptU;
...
begin
  ...
  StrGrid.MergeCells.AddRectXY(2,0,5,0);
  StrGrid.MergeCells.AddRectXY(6,0,9,0);
...
begin
  ...
  if Main.OpenDb then begin
    if Lokasi.Text<>'All' then begin
      ...
      StrLokasi:='0';
    QStr:='EXEC GetVhcOutDateTimeResume '+StrLokasi+',1,'+
    ...
    if Qry.RecordCount>0 then begin
      while Not(Qry.Eof) do begin
        ...
        IntTotal:=Qry.FieldValues['total_out1daya6']+Qry.FieldValues['total_out1dayb6']+Qry.FieldValues['total_out2daya6']+Qry.FieldValues['total_out2dayb6'];
        for Count:=2 to 10 do
          StrGrid.CellStyle[Count,IntRow].HorizontalAlignment:=taCenter;
        StrGrid.Cells[2,IntRow]:=Qry.FieldValues['total_out1dayb6'];
        if Qry.FieldValues['gross_out1dayb6']<>NULL then
          StrGrid.Cells[3,IntRow]:=IToCurr(Qry.FieldValues['gross_out1dayb6']);
        StrGrid.Cells[4,IntRow]:=Qry.FieldValues['total_out1daya6'];
        if Qry.FieldValues['gross_out1daya6']<>NULL then
          StrGrid.Cells[5,IntRow]:=IToCurr(Qry.FieldValues['gross_out1daya6']);
        StrGrid.Cells[6,IntRow]:=Qry.FieldValues['total_out2dayb6'];
        if Qry.FieldValues['gross_out2dayb6']<>NULL then
          StrGrid.Cells[7,IntRow]:=IToCurr(Qry.FieldValues['gross_out2dayb6']);
        StrGrid.Cells[8,IntRow]:=Qry.FieldValues['total_out2daya6'];
        if Qry.FieldValues['gross_out2daya6']<>NULL then
          StrGrid.Cells[9,IntRow]:=IToCurr(Qry.FieldValues['gross_out2daya6']);
        StrGrid.Cells[10,IntRow]:=IntToStr(IntTotal);
        Qry.Next;
      end;
    end;
    Qry.Close;
...
procedure TVhcOutTimeGrossMonthlyRpt.SelesaiClick(Sender: TObject);
begin
  VhcOutTimeGrossMonthlyRpt.Close;
end;

procedure TVhcOutTimeGrossMonthlyRpt.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TVhcOutTimeGrossMonthlyRpt.LokasiChange(Sender: TObject);
begin
