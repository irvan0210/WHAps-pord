...
var
  ReturBarangList: TReturBarangList;
...
constructor TReturBarangList.Create(AOwner:TComponent;Form_Request:String='';Form_Function:String='';Is_All:Integer=9;Is_Blok:Byte=0);
begin
  FormRequest:=Form_Request;
  Main.WriteLog('Form Open: ReturBarangList='+Form_Request);
  Inherited Create(AOwner);
end;

procedure TReturBarangList.Init;
...
procedure TReturBarangList.InitGrid;
...
procedure TReturBarangList.RefreshData;
...
begin
  ...
  if Main.OpenDb then begin
    ...
    StrQry:='SELECT retur_part_id,work_order_id,note '+
            ',CONVERT(VARCHAR(10),submit_date,103) AS tgl '+
            'FROM wh_retur_part '+
            'WHERE company_id='+StrCompanyId+' AND location_id='+StrLocationId+' and status=1';
    ...
    if Qry.RecordCount>0 then while not(Qry.Eof) do begin
      ...
      WorkOrderArr[IntCount][2]:=Qry.FieldValues['retur_part_id'];
      WorkOrderArr[IntCount][3]:=Qry.FieldValues['work_order_id'];
      WorkOrderArr[IntCount][6]:=Qry.FieldValues['note'];

      StrQry:='select * from wh_retur_part_detail where '+
            'retur_part_id='+QuotedStr(Qry.FieldValues['retur_part_id']);
      Qry2.SQL.Clear;
      Main.WriteLog('SQL :'+StrQry,2);
      Qry2.SQL.Add(StrQry);
      Qry2.Open;
      ...
      if Qry2.RecordCount>0 then while not(Qry2.Eof) do begin
        ...
        WorkOrderArr[IntCount][4]:=Qry2.FieldValues['part_name'];
        WorkOrderArr[IntCount][5]:=Qry2.FieldValues['qty'];
...
procedure TReturBarangList.RefreshGrid;
...
procedure TReturBarangList.RefreshCombo;
var Qry:TADOQuery;
    StrQry:String;
    IntCount:Integer;
begin
...
procedure TReturBarangList.FormShow(Sender: TObject);
begin
  Init;
  InitGrid;
  RefreshCombo;
...
procedure TReturBarangList.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
end;
