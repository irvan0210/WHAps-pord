...
uses MainU, SPJFormBusU, OrderFeeAddU;

{$R *.dfm}

constructor TSJBusRpt.Create(AOwner:TComponent;Form_Request:String='';Is_All:Integer=0;Is_Foward:Boolean=True;In_Out:Integer=0);
begin
  IsAll:=Is_All;
...
procedure TSJBusRpt.Init;
begin
  Cari.Text:='';
  if StrToInt(CompanyId)=1 then SBU.Enabled:=True else SBU.Enabled:=False;
end;

procedure TSJBusRpt.InitGrid;
...
procedure TSJBusRpt.RefreshCombo;
var Qry:TADOQuery;
    StrQry:String;
    IntCount:Integer;
begin
...
procedure TSJBusRpt.RefreshData;
...
procedure TSJBusRpt.RefreshGrid;
var IntCount:Integer;
begin
...
procedure TSJBusRpt.SelesaiClick(Sender: TObject);
begin
  Close;
end;

procedure TSJBusRpt.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TSJBusRpt.GridSPJSelectCell(Sender: TObject; ACol,
  ARow: Integer; var CanSelect: Boolean);
begin
  IntRow:=ARow;
end;

procedure TSJBusRpt.GridSPJDblClick(Sender: TObject);
begin
...
procedure TSJBusRpt.FormShow(Sender: TObject);
begin
  InitGrid;
...
procedure TSJBusRpt.CariChange(Sender: TObject);
...
procedure TSJBusRpt.ToXCelClick(Sender: TObject);
begin
...
procedure TSJBusRpt.AllFowardClick(Sender: TObject);
begin
...
procedure TSJBusRpt.AllClick(Sender: TObject);
begin
  if not(Initiation) then begin
    RefreshData;
    RefreshGrid;
...
procedure TSJBusRpt.FowardClick(Sender: TObject);
begin
  if not(Initiation) then begin
    RefreshData;
    RefreshGrid;
...
procedure TSJBusRpt.BackwardClick(Sender: TObject);
begin
  if not(Initiation) then begin
    RefreshData;
    RefreshGrid;
