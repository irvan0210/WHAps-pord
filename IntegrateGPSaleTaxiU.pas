...
type
  TIntegrateGPSaleTaxi = class(TForm)
...
var
  IntegrateGPSaleTaxi: TIntegrateGPSaleTaxi;
...
procedure TIntegrateGPSaleTaxi.Init;
begin
  FromDates.Date:=Now();
  Lokasi.ItemIndex:=-1;
  LokasiDisp.Text:='';
...
procedure TIntegrateGPSaleTaxi.InitGrid1;
...
procedure TIntegrateGPSaleTaxi.InitGrid2;
var IntCount,IntCount2:Integer;
begin
  for IntCount:=0 to StrGrid2.RowCount-1 do
    for IntCount2:=0 to StrGrid2.ColCount-1 do
...
procedure TIntegrateGPSaleTaxi.RefreshJenis;
var StrQry:String;
    Qry:TADOQuery;
    IntCount:Integer;
begin
...
procedure TIntegrateGPSaleTaxi.RefreshCombo;
var QStr:String;
    Qry:TADOQuery;
    Count:Integer;
begin
...
procedure TIntegrateGPSaleTaxi.RefreshData1;
...
begin
  ...
  if Main.OpenDb then begin
    ...
    StrQry:='EXEC IntegrateGPTX1 '+QuotedStr(FormatDateTime('yyyy/mm/dd',FromDates.Date))+','+QuotedStr(Lokasi.Text)+',@Total=1'+';';
    Qry.SQL.Add(StrQry);
    Qry.Open;
    ...
    Qry.Close;
    StrQry:='EXEC IntegrateGPTX2 '+QuotedStr(FormatDateTime('yyyy/mm/dd',FromDates.Date))+','+QuotedStr(Lokasi.Text)+';';
    Qry.SQL.Clear;
    Qry.SQL.Add(StrQry);
    Qry.Open;
    IntCount:=0;
    ...
    Qry.Close;
{    if StrTotal<>'' then begin
      StrGrid1.RowCount:=StrGrid1.RowCount+1;
      StrGrid1.Cells[0,IntCount+7]:=GPIndex.Text;
      StrGrid1.Cells[1,IntCount+7]:=LokasiArr[Lokasi.ItemIndex][3];
      StrGrid1.Cells[2,IntCount+7]:='0,00';
      StrGrid1.Cells[3,IntCount+7]:=StrTotal;
      StrGrid1.Cells[4,IntCount+7]:=StrReference;
      StrGrid1.Cells[5,IntCount+7]:='1';
    end;
}
  end;
...
procedure TIntegrateGPSaleTaxi.RefreshData2;
var Qry,Qry2:TADOQuery;
    StrQry,StrBatchId,StrGLNum,StrLastIndex1,StrLastIndex2:String;
    IntCount,IntCount2:Integer;
begin
  Main.M_Busy;
  Qry:=TADOQuery.Create(Self);
  ...
  if Main.OpenDb then begin
    ...
    IntCount:=1;
    StrQry:='EXEC IntegrateGPTX1 '+QuotedStr(FormatDateTime('yyyy/mm/dd',FromDates.Date))+','+QuotedStr(Lokasi.Text)+',@Total=0'+';';
    Qry.SQL.Add(StrQry);
    Qry.Open;
    if Qry.RecordCount>0 then begin
      if Qry.FieldValues['total']<>NULL then begin
    ...
    StrLastIndex1:='';
    StrLastIndex2:='';
    StrQry:='EXEC IntegrateGPTX4 '+QuotedStr(FormatDateTime('yyyy/mm/dd',FromDates.Date))+','+QuotedStr(Lokasi.Text)+';';
    Qry.SQL.Clear;
    Qry.SQL.Add(StrQry);
    Qry.Open;
    if Qry.RecordCount>0 then while not(Qry.Eof) do begin
    ...
    Qry.Close;
    Inc(IntCount);
    StrQry:='EXEC IntegrateGPTX3 '+QuotedStr(FormatDateTime('yyyy/mm/dd',FromDates.Date))+','+QuotedStr(Lokasi.Text)+';';
    Qry.SQL.Clear;
    Qry.SQL.Add(StrQry);
    Qry.Open;
    IntCount2:=1;
    if Qry.RecordCount>0 then while not Qry.Eof do begin
      ...
      StrGrid2.Cells[0,17+IntCount+(IntCount2*2)]:=StrBatchId;
      StrGrid2.Cells[1,17+IntCount+(IntCount2*2)]:=GPLastGL.Text;
      StrGrid2.Cells[2,17+IntCount+(IntCount2*2)]:=StrGLNum;
      if Qry.FieldValues['coa_disposisi']<>NULL then
        StrGrid2.Cells[3,17+IntCount+(IntCount2*2)]:=Qry.FieldValues['coa_disposisi'];
      if Qry.FieldValues['amount_debit']<>NULL then
        StrGrid2.Cells[4,17+IntCount+(IntCount2*2)]:=IntToStr(Qry.FieldValues['amount_debit'])+',00';
      if Qry.FieldValues['amount_credit']<>NULL then
        StrGrid2.Cells[5,17+IntCount+(IntCount2*2)]:=IntToStr(Qry.FieldValues['amount_credit'])+',00';
      if Qry.FieldValues['row']<>NULL then
        StrGrid2.Cells[6,17+IntCount+(IntCount2*2)]:=IntToStr(Qry.FieldValues['row']);
      if Qry.FieldValues['amount_dimension']<>NULL then
        StrGrid2.Cells[7,17+IntCount+(IntCount2*2)]:=IntToStr(Qry.FieldValues['amount_dimension'])+',00';
      StrGrid2.Cells[8,17+IntCount+(IntCount2*2)]:='BODY';
      StrGrid2.Cells[9,17+IntCount+(IntCount2*2)]:=Qry.FieldValues['body_id'];
      if Qry.FieldValues['details']<>NULL then
        StrGrid2.Cells[10,17+IntCount+(IntCount2*2)]:=Qry.FieldValues['details'];
      if Qry.FieldValues['amount_credit']>0 then StrGrid2.Cells[11,17+IntCount+(IntCount2*2)]:=StrLastIndex1
      else StrGrid2.Cells[11,17+IntCount+(IntCount2*2)]:=StrLastIndex2;
      StrGrid2.RowCount:=StrGrid2.RowCount+1;
      StrGrid2.Cells[0,18+IntCount+(IntCount2*2)]:=StrBatchId;
      StrGrid2.Cells[1,18+IntCount+(IntCount2*2)]:=GPLastGL.Text;
      StrGrid2.Cells[2,18+IntCount+(IntCount2*2)]:=StrGLNum;
      if Qry.FieldValues['coa_disposisi']<>NULL then
        StrGrid2.Cells[3,18+IntCount+(IntCount2*2)]:=Qry.FieldValues['coa_disposisi'];
      if Qry.FieldValues['amount_debit']<>NULL then
        StrGrid2.Cells[4,18+IntCount+(IntCount2*2)]:=IntToStr(Qry.FieldValues['amount_debit'])+',00';
      if Qry.FieldValues['amount_credit']<>NULL then
        StrGrid2.Cells[5,18+IntCount+(IntCount2*2)]:=IntToStr(Qry.FieldValues['amount_credit'])+',00';
      if Qry.FieldValues['row']<>NULL then
        StrGrid2.Cells[6,18+IntCount+(IntCount2*2)]:=IntToStr(Qry.FieldValues['row']);
      if Qry.FieldValues['amount_dimension']<>NULL then
        StrGrid2.Cells[7,18+IntCount+(IntCount2*2)]:=IntToStr(Qry.FieldValues['amount_dimension'])+',00';
      StrGrid2.Cells[8,18+IntCount+(IntCount2*2)]:='POOL';
      StrGrid2.Cells[9,18+IntCount+(IntCount2*2)]:=Qry.FieldValues['code'];
      if Qry.FieldValues['details']<>NULL then
        StrGrid2.Cells[10,18+IntCount+(IntCount2*2)]:=Qry.FieldValues['details'];
      if Qry.FieldValues['amount_credit']>0 then StrGrid2.Cells[11,18+IntCount+(IntCount2*2)]:=StrLastIndex1
      else StrGrid2.Cells[11,18+IntCount+(IntCount2*2)]:=StrLastIndex2;
      Qry.Next;
      Inc(IntCount2);
    end;
    Qry.Close;
...
procedure TIntegrateGPSaleTaxi.FormShow(Sender: TObject);
begin
  Init;
  RefreshCombo;
  RefreshJenis;
...
procedure TIntegrateGPSaleTaxi.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TIntegrateGPSaleTaxi.SelesaiClick(Sender: TObject);
begin
  Close;
end;

procedure TIntegrateGPSaleTaxi.ToXCel1Click(Sender: TObject);
begin
  if ToExcel4(StrGrid1) then ShowMessage('Export ke Excel Berhasil')
  else ShowMessage('Export ke Excel Gagal');
end;

procedure TIntegrateGPSaleTaxi.ToXcel2Click(Sender: TObject);
begin
...
procedure TIntegrateGPSaleTaxi.ProcessClick(Sender: TObject);
begin
  ProcessData;
end;

procedure TIntegrateGPSaleTaxi.ProcessData;
begin
  InitGrid1;
  InitGrid2;
  RefreshData1;
...
procedure TIntegrateGPSaleTaxi.GPLastGLKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Not(Key In ['0'..'9',#8,#13]) then Key:=#0;
end;

procedure TIntegrateGPSaleTaxi.LokasiChange(Sender: TObject);
begin
  LokasiDisp.Text:=LokasiArr[Lokasi.ItemIndex][1];  
end;
