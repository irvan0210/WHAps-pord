unit KetidakSesuaianDanPerbaikanListU;
...
type
  TKetidakSesuaianDanPerbaikanList = class(TForm)
    Label2: TLabel;
    ToXCel: TSpeedButton;
    Selesai: TButton;
...
var
  KetidakSesuaianDanPerbaikanList: TKetidakSesuaianDanPerbaikanList;

implementation

uses
  MainU, CetakKetidaksesuaianDanPerbaikanU;

{$R *.dfm}
constructor TKetidakSesuaianDanPerbaikanList.Create(AOwner:TComponent;Form_Request:String='';Customer_Id:String='';Tipe_Data:Integer=0;Is_Closed:Byte=0);
begin
...
procedure TKetidakSesuaianDanPerbaikanList.Init;
begin
  Cari.Text:='';
end;

procedure TKetidakSesuaianDanPerbaikanList.InitGrid;
...
begin
  ...
  GridCCP.ColWidths[1]:=168;
  GridCCP.ColWidths[2]:=100;
  GridCCP.ColWidths[3]:=100;
  GridCCP.ColWidths[4]:=65;
  ...
  GridCCP.ColWidths[22]:=0;

  GridCCP.SizingHeight:=True;

  GridCCP.Cells[0,0]:='Tanggal';
  GridCCP.Cells[1,0]:='No Ketidak Sesuain dan Perbaikan';
  GridCCP.Cells[2,0]:='No Komplain';
...
procedure TKetidakSesuaianDanPerbaikanList.RefreshCombo;
var Qry:TADOQuery;
    StrQry:String;
    IntCount:Integer;
begin
...
procedure TKetidakSesuaianDanPerbaikanList.RefreshData;
...
begin
  ...
  if Main.OpenDb then begin
    ...
    if (Qry.RecordCount>0) then while not(Qry.Eof) do begin
       ...
       CCPArr[IntCount][0]:=Qry.FieldValues['tgl_ketidaksesuaian_perbaikan'];
       CCPArr[IntCount][1]:=Qry.FieldValues['no_ketidaksesuaian_perbaikan'];
       CCPArr[IntCount][2]:=Qry.FieldValues['cust_complain_id'];
       ...
       CCPArr[IntCount][22]:=Qry.FieldValues['is_closed'];
       Inc(IntCount);
      Qry.Next;
    end;
    Qry.Close;
...
procedure TKetidakSesuaianDanPerbaikanList.RefreshGrid;
var IntCount:Integer;
begin
  ...
  for IntCount:=0 to Length(CCPArr)-1 do begin
    ...
    GridCCP.Cells[22,IntCount+1]:=CCPArr[IntCount][22];
...
procedure TKetidakSesuaianDanPerbaikanList.SelesaiClick(Sender: TObject);
begin
  Main.M_Normal;
  Close;
end;

procedure TKetidakSesuaianDanPerbaikanList.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TKetidakSesuaianDanPerbaikanList.FormShow(Sender: TObject);
begin
  InitGrid;
...
procedure TKetidakSesuaianDanPerbaikanList.GridCCPSelectCell(
  Sender: TObject; ACol, ARow: Integer; var CanSelect: Boolean);
begin
  IntRow:=ARow;
end;

procedure TKetidakSesuaianDanPerbaikanList.GridCCPDblClick(
  Sender: TObject);
begin
  if (IntRow>0) AND (GridCCP.Cells[1,IntRow]<>'') then begin
    if UpperCase(FormRequest)='MAIN-CETAKULANG' then  begin
      if Main.IsFormOpen('CetakKetidaksesuaianDanPerbaikan')=False then
      begin
        StrTransId:=GridCCP.Cells[1,IntRow];
        IsClose:=StrToInt(GridCCP.Cells[22,IntRow]);
        CetakKetidaksesuaianDanPerbaikan:=TCetakKetidaksesuaianDanPerbaikan.Create(Self,'VIEW','');
...
procedure TKetidakSesuaianDanPerbaikanList.LihatClick(Sender: TObject);
begin
  RefreshData;
  RefreshGrid;
end;
