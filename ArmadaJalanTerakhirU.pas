...
type
  TArmadaJalanTerakhir = class(TForm)
    Label1: TLabel;
    Label3: TLabel;
    Label6: TLabel;
    Label13: TLabel;
...
//    procedure CariChange(Sender: TObject);
  ...
  private
    ...
    TransId,EmplId,VehicleId,StrIsOnline, StrVhcTransId:String;
    IsDataFound, Initiation,IsReadOnly,IsInput:Boolean;
...
//    procedure InitGridSJ;
  ...
  public
    ...
    constructor Create(AOwner:TComponent;VhcTrans_Id:String='';IsRead_Only:Boolean=False);overload;
  end;

var
  ArmadaJalanTerakhir: TArmadaJalanTerakhir;
...
uses MainU, Math, DateUtils, VehicleEquipmentCheckU
  , IntersysAPI_New
  , AuthorizedFormU, LoginU, DB, CheckerInU;


{$R *.dfm}

constructor TArmadaJalanTerakhir.Create(AOwner:TComponent;VhcTrans_Id:String='';IsRead_Only:Boolean=False);
begin
  StrVhcTransId:=VhcTrans_Id;
  Initiation:=True;
  IsReadOnly:=IsRead_Only;
  Main.WriteLog('Form Open: ArmadaJalanTerakhir='+VhcTrans_Id+','+BoolToStr(IsRead_Only));
  //FormRequest:=Form_Request;
  inherited Create(AOwner);
end;

procedure TArmadaJalanTerakhir.RefreshHeader;
var StrQry:String;
    Qry:TADOQuery;
begin
  Main.M_Busy;
...
procedure TArmadaJalanTerakhir.Init;
begin
  NoSJ.Items.Clear;
  NoSJ.Text:='';
  NoSJ.ItemIndex:=-1;
...
procedure TArmadaJalanTerakhir.InitGridService;
...
procedure TArmadaJalanTerakhir.RefreshData;
...
{procedure TArmadaJalanTerakhir.RefreshGridSJ;
...
procedure TArmadaJalanTerakhir.CheckData;
...
//            InTime.Text:=FormatDateTime('HH:NN:SS',Qry.FieldValues['in_time']);
    ...
    //  StrQry:='EXEC GetCustomerOrderServiceList '+QuotedStr(SJArr[ArrayIndexOf(SJArr,NoSJ.Text,0)][20])+';';
      StrQry:='EXEC GetCustomerOrderServiceList '+QuotedStr(Cari.Text)+';';
      Qry.SQL.Clear;
      Main.WriteLog('SQL :'+StrQry,2);
      Qry.SQL.Add(StrQry);
      Qry.Open;
...
procedure TArmadaJalanTerakhir.BersihkanClick(Sender: TObject);
begin
  Main.M_Busy;
  Cari.Text:='';
  Init;
...
procedure TArmadaJalanTerakhir.FormShow(Sender: TObject);
begin
  Init;
  InitGridService;
  RefreshHeader;
  ...
  Cari.Text:= StrVhcTransId;
  //Cari.SetFocus;
  //if (NoBodi.Text='')  then begin      // AND (Trim(GridSPJ.Cells[1,IntRow])<>'')
   // MessageBox(0,PChar(StrVhcTransId),'Armada Jalan Terakhir',MB_OK or MB_ICONWARNING);;
  //  NoSJ.ItemIndex:=NoSJ.Items.IndexOf(QuotedStr(StrVhcTransId));
  //end;

end;

procedure TArmadaJalanTerakhir.DisableInput;
begin
  NoSJ.Enabled:=False;
...
procedure TArmadaJalanTerakhir.EnableInput;
begin
  NoSJ.Enabled:=True;
...
procedure TArmadaJalanTerakhir.SelesaiClick(Sender: TObject);
begin
  Main.StatusUpdate('','');
  ArmadaJalanTerakhir.Close;
end;

procedure TArmadaJalanTerakhir.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TArmadaJalanTerakhir.KMOdoKeyPress(Sender: TObject; var Key: Char);
begin
  if Not(Key In ['0'..'9',#8,#13]) then Key:=#0;
  if Key=#13 then Simpan.SetFocus;
end;

procedure TArmadaJalanTerakhir.NoSJChange(Sender: TObject);
begin
  Main.M_Busy;
  CheckData;
  Main.M_Normal;
...
procedure TArmadaJalanTerakhir.KmOdoAkhirEnter(Sender: TObject);
begin
  KmOdoAkhir.Text:=ToString(KmOdoAkhir.Text);
end;

procedure TArmadaJalanTerakhir.KmOdoAkhirExit(Sender: TObject);
begin
  KmOdoAkhir.Text:=ToString(KmOdoAkhir.Text);
...
procedure TArmadaJalanTerakhir.KmOdoAkhirKeyPress(Sender: TObject; var Key: Char);
begin
  if Not(Key In ['0'..'9',#8,#13,#3,#22]) then Key:=#0;
  if Key=#13 then BBMPrice.SetFocus;
end;

procedure TArmadaJalanTerakhir.SimpanClick(Sender: TObject);
...
procedure TArmadaJalanTerakhir.InTimeKeyPress(Sender: TObject; var Key: Char);
begin
  if Key=#13 then RemarkOut.SetFocus;
end;

procedure TArmadaJalanTerakhir.RemarkOutKeyPress(Sender: TObject; var Key: Char);
begin
  if Key=#13 then Simpan.SetFocus;
end;

{procedure TArmadaJalanTerakhir.CariChange(Sender: TObject);
...
procedure TArmadaJalanTerakhir.InDateKeyPress(Sender: TObject; var Key: Char);
begin
  if Key=#13 then InTime.SetFocus;
end;

procedure TArmadaJalanTerakhir.BBMPriceKeyPress(Sender: TObject; var Key: Char);
begin
  if Not(Key In ['0'..'9',#8,#13,#3,#22]) then Key:=#0;
  if Key=#13 then BBMPriceReimburse.SetFocus;
end;

procedure TArmadaJalanTerakhir.BBMPriceExit(Sender: TObject);
begin
...
procedure TArmadaJalanTerakhir.AllClick(Sender: TObject);
begin
 // RefreshData;
 // RefreshGridSJ;
end;

procedure TArmadaJalanTerakhir.CheckListClick(Sender: TObject);
begin
...
procedure TArmadaJalanTerakhir.TolParkirEnter(Sender: TObject);
begin
  TolParkir.Text:=ToString(TolParkir.Text);
end;

procedure TArmadaJalanTerakhir.BBMPriceEnter(Sender: TObject);
begin
  BBMPrice.Text:=ToString(BBMPrice.Text);
end;

procedure TArmadaJalanTerakhir.BBMLiterEnter(Sender: TObject);
begin
  BBMLiter.Text:=ToString(BBMLiter.Text); 
end;

procedure TArmadaJalanTerakhir.BBMLiterExit(Sender: TObject);
begin
...
procedure TArmadaJalanTerakhir.TolParkirKeyPress(Sender: TObject; var Key: Char);
begin
  if Not(Key In ['0'..'9',#8,#13]) then Key:=#0;
  if Key=#13 then eOvertime.SetFocus;
end;

procedure TArmadaJalanTerakhir.TolParkirExit(Sender: TObject);
begin
  if ToString(TolParkir.Text)='' then TolParkir.Text:='0';
  TolParkir.Text:=SToCurr(TolParkir.Text);
end;

procedure TArmadaJalanTerakhir.BBMLiterKeyPress(Sender: TObject; var Key: Char);
begin
  if Not(Key In ['0'..'9',#8,#13,#3,#22]) then Key:=#0;
  if Key=#13 then BBMPriceReimburse.SetFocus;
end;

procedure TArmadaJalanTerakhir.BBMPriceReimburseEnter(Sender: TObject);
begin
  BBMPriceReimburse.Text:=ToString(BBMPriceReimburse.Text);
end;

procedure TArmadaJalanTerakhir.BBMPriceReimburseExit(Sender: TObject);
begin
...
procedure TArmadaJalanTerakhir.BBMPriceReimburseKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Not(Key In ['0'..'9',#8,#13]) then Key:=#0;
...
procedure TArmadaJalanTerakhir.BBMLiterReimburseEnter(Sender: TObject);
begin
  BBMLiterReimburse.Text:=ToString(BBMLiterReimburse.Text);
end;

procedure TArmadaJalanTerakhir.BBMLiterReimburseExit(Sender: TObject);
begin
...
procedure TArmadaJalanTerakhir.BBMLiterReimburseKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Not(Key In ['0'..'9',#8,#13]) then Key:=#0;
  if Key=#13 then TolParkir.SetFocus;
...
procedure TArmadaJalanTerakhir.DriverReimburseExit(Sender: TObject);
begin
  if ToString(DriverReimburse.Text)='' then DriverReimburse.Text:='0';
  DriverReimburse.Text:=SToCurr(DriverReimburse.Text);
end;

procedure TArmadaJalanTerakhir.DriverReimburseEnter(Sender: TObject);
begin
  DriverReimburse.Text:=ToString(DriverReimburse.Text);
end;

procedure TArmadaJalanTerakhir.DriverReimburseKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Not(Key In ['0'..'9',#8,#13]) then Key:=#0;
...
procedure TArmadaJalanTerakhir.eOvertimeChange(Sender: TObject);
begin
//
end;

procedure TArmadaJalanTerakhir.eOvertimeKeyPress(Sender: TObject; var Key: Char);
begin
  if Not(Key In ['0'..'9',#8,#13]) then Key:=#0;
  if Key=#13 then InDate.SetFocus;
end;

procedure TArmadaJalanTerakhir.eOvertimeEnter(Sender: TObject);
begin
  eOvertime.Text:=ToString(eOvertime.Text);
end;

procedure TArmadaJalanTerakhir.eOvertimeExit(Sender: TObject);
begin
  if ToString(eOvertime.Text)='' then eOvertime.Text:='0';
  eOvertime.Text:=SToCurr(eOvertime.Text);
end;

procedure TArmadaJalanTerakhir.RePrintReimburse(Trans_Id:String);
var Qry:TADOQuery;
    QStr:String;
...
procedure TArmadaJalanTerakhir.ZColorStringGrid1DblClick(Sender: TObject);
begin
  if (NoBodi.Text='')  then begin      // AND (Trim(GridSPJ.Cells[1,IntRow])<>'')
    Main.M_Busy;
    NoSJ.ItemIndex:=NoSJ.Items.IndexOf(StrVhcTransId);
    CheckData;
    Main.M_Normal;
  end;
end;

procedure TArmadaJalanTerakhir.btn1Click(Sender: TObject);
begin
...
procedure TArmadaJalanTerakhir.KMOdoAwalEnter(Sender: TObject);
begin
  KmOdoAwal.Text:=ToString(KmOdoAwal.Text);
end;

procedure TArmadaJalanTerakhir.KMOdoAwalExit(Sender: TObject);
begin
  KmOdoAwal.Text:=ToString(KmOdoAwal.Text);
...
procedure TArmadaJalanTerakhir.GridSPJSelectCell(Sender: TObject; ACol,
