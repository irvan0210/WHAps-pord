...
type
  TRevenueVhcDayBusRpt = class(TForm)
    Selesai: TButton;
    ToXCel: TSpeedButton;
    Batch: TComboBox;
    Label3: TLabel;
...
var
  RevenueVhcDayBusRpt: TRevenueVhcDayBusRpt;
...
procedure TRevenueVhcDayBusRpt.Init;
...
//  Batch.Text:='All';
  ...
  StrGrid.MergeCells.AddRectXY(33,0,33,1);
  StrGrid.MergeCells.AddRectXY(34,0,34,1);
  StrGrid.MergeCells.AddRectXY(35,0,35,1);
  StrGrid.MergeCells.AddRectXY(36,0,36,1);
  ...
  StrGrid.Cells[33,0]:='Lain2'; {29}
  StrGrid.Cells[34,0]:='Total Biaya';

  StrGrid.Cells[35,0]:='KM Out';
  StrGrid.Cells[36,0]:='KM In';
  StrGrid.Cells[37,0]:='KM Run';
  StrGrid.Cells[38,0]:='R BBM';
  StrGrid.Cells[39,0]:='Inv-Biaya';
  StrGrid.Cells[40,0]:='Keterangan';
  StrGrid.Cells[41,0]:='No E-Toll';
  ...
  StrGrid.ColWidths[34]:=90;

  StrGrid.ColWidths[35]:=50;
  StrGrid.ColWidths[36]:=50;
  StrGrid.ColWidths[37]:=50;
  StrGrid.ColWidths[38]:=50;
  StrGrid.ColWidths[39]:=70;
  StrGrid.ColWidths[40]:=150;
  StrGrid.ColWidths[41]:=140;
...
procedure TRevenueVhcDayBusRpt.RefreshCombo;
var StrQry:String;
    Qry:TADOQuery;
    IntCount:Integer;
begin
...
//      if Qry.FieldValues['in_time']<>NULL then StrGrid.Cells[14,Count]:=Qry.FieldValues['in_time'];
      ...
      if Qry.FieldValues['in_ordo_km']<>NULL then begin
        StrGrid.Cells[36,Count]:=IToCurr(Qry.FieldValues['in_ordo_km']);
        StrGrid.Cells[37,Count]:=IToCurr(Qry.FieldValues['distance']);
        if (Qry.FieldValues['fuel_litre']>0) then
          StrGrid.Cells[38,Count]:=FloatToStrF(Qry.FieldValues['distance']/StrToFloat(Qry.FieldValues['fuel_litre']),ffNumber,6,2)
        else
          StrGrid.Cells[38,Count]:='0';
      end else begin
        StrGrid.Cells[36,Count]:='';
        StrGrid.Cells[37,Count]:='0';
        StrGrid.Cells[38,Count]:='0';
      end;
      if Qry.FieldValues['description']<>NULL then StrGrid.Cells[40,Count]:=Qry.FieldValues['description'];
      if Qry.FieldValues['etoll_number']<>NULL then StrGrid.Cells[41,Count]:=eToll(Qry.FieldValues['etoll_number']);
  ...
  StrGrid.Cells[33,StrGrid.RowCount-1]:=IToCurr(TotalLain);
  StrGrid.Cells[34,StrGrid.RowCount-1]:=IToCurr(IntTotalBiaya);

  StrGrid.Cells[39,StrGrid.RowCount-1]:=IToCurr(IntTotalGrossProfit);
...
procedure TRevenueVhcDayBusRpt.SelesaiClick(Sender: TObject);
begin
  RevenueVhcDayBusRpt.Close;
end;

procedure TRevenueVhcDayBusRpt.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TRevenueVhcDayBusRpt.RefreshClick(Sender: TObject);
begin
  RefreshData;
end;

procedure TRevenueVhcDayBusRpt.ToXCelClick(Sender: TObject);
begin
  if ToExcel4(StrGrid) then ShowMessage('Export ke Excel Berhasil')
    else ShowMessage('Export ke Excel Gagal');
end;

procedure TRevenueVhcDayBusRpt.FormShow(Sender: TObject);
begin
  Init;
  RefreshCombo;
  Initiation:=False;
...
procedure TRevenueVhcDayBusRpt.BatchChange(Sender: TObject);
begin
  GroupDisp.Text:=GroupArr[Batch.ItemIndex][1];
end;

procedure TRevenueVhcDayBusRpt.StrGridDblClick(Sender: TObject);
var IsAuth:Boolean;
begin
...
procedure TRevenueVhcDayBusRpt.StrGridSelectCell(Sender: TObject; ACol,
...
procedure TRevenueVhcDayBusRpt.CekTglSampaiClick(Sender: TObject);
begin
...
procedure TRevenueVhcDayBusRpt.FormCreate(Sender: TObject);
begin
    Initiation:=True;
end;

procedure TRevenueVhcDayBusRpt.Copy1Click(Sender: TObject);
var
  S: string;
  GRect: TGridRect;
  C, R: Integer;
...
procedure TRevenueVhcDayBusRpt.StrGridKeyDown(Sender: TObject;
