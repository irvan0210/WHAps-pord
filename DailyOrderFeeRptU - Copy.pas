...
type
  ...
  private
    ...
    OrderFeeArr:Array of TArrString50;
    MaxCol:Integer;
...
begin
  ...
  StrGrid.MergeCells.AddRectXY(38+IntGeserKolom,0,38+IntGeserKolom,1);
  StrGrid.MergeCells.AddRectXY(39+IntGeserKolom,0,39+IntGeserKolom,1);
  StrGrid.MergeCells.AddRectXY(41+IntGeserKolom,0,41+IntGeserKolom,0);
  ...
  StrGrid.Cells[29,0]:='Total          (Exc Tol)';
  ...
  StrGrid.Cells[41,0]:='Waktu Topup';
  StrGrid.Cells[42,0]:='Jam Topup';
  StrGrid.ColWidths[42]:=0;

  StrGrid.Cells[43,0]:='No Rek Driver';
  StrGrid.Cells[44,0]:='Nama di Rek';
  StrGrid.Cells[45,0]:='Keberangkatan';

  StrGrid.Cells[46,0]:='Armada';
  StrGrid.Cells[46,1]:='Jenis';
  StrGrid.Cells[47,1]:='Seat';
  StrGrid.Cells[48,0]:='Jam Selesai Tamu';

  StrGrid.ColWidths[46]:=120;
  StrGrid.ColWidths[47]:=35;
  StrGrid.ColWidths[48]:=55;
...
begin
  ...
  for IntCount:=0 to Length(OrderFeeArr)-1 do begin
    ...
    if (OrderFeeArr[IntCount][44]<>NULL) or (OrderFeeArr[IntCount][44]<>'') then begin  {employee_id}
      ...
      TotalTolParkir:=TotalTolParkir+StoInt(OrderFeeArr[IntCount][18]);
      TotalTol:=TotalTol+StoInt(OrderFeeArr[IntCount][19]);
      ...
      TotalLain:=TotalLain+StoInt(OrderFeeArr[IntCount][24]);
      ...
      TotalBiayaExc:=TotalBiayaExc+IntBiaya+IntBiayaReimburse+StoInt(OrderFeeArr[IntCount][24]);
      ...
      StrGrid.Cells[22,Count]:=IToCurr(StoInt(OrderFeeArr[IntCount][18]));
      StrGrid.Cells[23,Count]:=IToCurr(StoInt(OrderFeeArr[IntCount][22]));

      StrGrid.Cells[24,Count]:=IToCurr(StoInt(OrderFeeArr[IntCount][19]));
      ...
      StrGrid.Cells[29,Count]:=IToCurr(IntBiaya+IntBiayaReimburse+StoInt(OrderFeeArr[IntCount][24]));
      ...
      if OrderFeeArr[IntCount][46]<>NULL then StrGrid.Cells[41,Count]:=eToll(OrderFeeArr[IntCount][46]);
      if OrderFeeArr[IntCount][47]<>NULL then StrGrid.Cells[42,Count]:=eToll(OrderFeeArr[IntCount][47]);

      if OrderFeeArr[IntCount][38]<>'' then StrGrid.Cells[43,Count]:=OrderFeeArr[IntCount][38];
      if OrderFeeArr[IntCount][39]>'' then StrGrid.Cells[44,Count]:=OrderFeeArr[IntCount][39];
      if OrderFeeArr[IntCount][40]<>'' then StrGrid.Cells[45,Count]:=OrderFeeArr[IntCount][40];

      if OrderFeeArr[IntCount][41]<>'' then StrGrid.Cells[46,Count]:=OrderFeeArr[IntCount][41];
      if OrderFeeArr[IntCount][42]<>'' then StrGrid.Cells[47,Count]:=OrderFeeArr[IntCount][42];
      if OrderFeeArr[IntCount][43]<>'' then StrGrid.Cells[48,Count]:=OrderFeeArr[IntCount][43];
    end;
    Inc(Count);
...
var QStr,StrBatch,StrLocationId,StrCompanyId,StrToDates,StrisAll,StrCancel:String;
...
begin
  ...
  QStr:='EXEC GetRevenueVhcDayRpt2 '+StrLocationId+','+
        ...
        QuotedStr(StrBatch)+','+StrCompanyId+StrToDates+',@Ordered=vhc_trans_id,@isAll='+StrisAll+StrCancel+';';
  Qry.SQL.Clear;
  Qry.SQL.Add(QStr);
  Qry.Open;
  ...
  if Qry.RecordCount>0 then while not (Qry.Eof) do begin
    ...
    if Qry.FieldValues['employee_id']<>NULL then begin
      ...
      TotalBiaya:=TotalBiaya+IntBiaya+IntBiayaReimburse+Qry.FieldValues['lain_lain']+Qry.FieldValues['tol'];

      TotalBiayaExc:=TotalBiayaExc+IntBiaya+IntBiayaReimburse+Qry.FieldValues['lain_lain'];
      ...
      {Geser 6 kolom}
      StrGrid.Cells[18,Count]:=IToCurr(Qry.FieldValues['fee_driver']*Qry.FieldValues['day']);
      StrGrid.Cells[19,Count]:=IToCurr(Qry.FieldValues['fee_driver_reimburse']);//*Qry.FieldValues['day']

      StrGrid.Cells[20,Count]:=IToCurr(Qry.FieldValues['fee_busboy']*Qry.FieldValues['day']);
      StrGrid.Cells[21,Count]:=IToCurr(Qry.FieldValues['fee_busboy_reimburse']);//*Qry.FieldValues['day']

      StrGrid.Cells[22,Count]:=IToCurr(Qry.FieldValues['tol_parkir']);
      StrGrid.Cells[23,Count]:=IToCurr(Qry.FieldValues['parkir_reimburse']);

      StrGrid.Cells[24,Count]:=IToCurr(Qry.FieldValues['tol']);
      StrGrid.Cells[25,Count]:=IToCurr(Qry.FieldValues['tol_reimburse']);

      //if Qry.FieldValues['etoll_number']<>NULL then StrGrid.Cells[16,Count]:='';//eToll(Qry.FieldValues['etoll_number']);
      StrGrid.Cells[26,Count]:=IToCurr(Qry.FieldValues['lain_lain']);
      StrGrid.Cells[27,Count]:=IToCurr(Qry.FieldValues['overtime']);
      StrGrid.Cells[28,Count]:=IToCurr(IntBiaya+IntBiayaReimburse+Qry.FieldValues['lain_lain']+Qry.FieldValues['tol']);
      StrGrid.Cells[29,Count]:=IToCurr(IntBiaya+IntBiayaReimburse+Qry.FieldValues['lain_lain']);
      StrGrid.Cells[30,Count]:=Qry.FieldValues['from_dates'];

      if Qry.FieldValues['out_source']<>NULL then begin
        StrGrid.CellStyle[30,Count].Font.Color:=clBlue;
        StrGrid.CellStyle[31,Count].Font.Color:=clBlue;
      end else begin
        StrGrid.CellStyle[30,Count].Font.Color:=clBlack;
      ...
      if Qry.FieldValues['out_time']<>NULL then StrGrid.Cells[31,Count]:=Qry.FieldValues['out_time'];

      if Qry.FieldValues['in_source']<>NULL then begin
        StrGrid.CellStyle[32,Count].Font.Color:=clBlue;
        StrGrid.CellStyle[33,Count].Font.Color:=clBlue;
      end else begin
        StrGrid.CellStyle[32,Count].Font.Color:=clBlack;
      ...
      if Qry.FieldValues['in_dates']<>NULL then StrGrid.Cells[32,Count]:=Qry.FieldValues['in_dates'];
      if Qry.FieldValues['in_time']<>NULL then StrGrid.Cells[33,Count]:=Qry.FieldValues['in_time'];
      if Qry.FieldValues['out_ordo_km']<>NULL then StrGrid.Cells[34,Count]:=IToCurr(Qry.FieldValues['out_ordo_km']);
      if Qry.FieldValues['in_ordo_km']<>NULL then begin
        StrGrid.Cells[35,Count]:=IToCurr(Qry.FieldValues['in_ordo_km']);
        StrGrid.Cells[36,Count]:=IToCurr(Qry.FieldValues['distance']);
        if (Qry.FieldValues['fuel_litre_budget'])+(Qry.FieldValues['fuel_litre'])+(Qry.FieldValues['fuel_litre_spbu']) >0 then
          //StrGrid.Cells[32,Count]:=FloatToStrF(Qry.FieldValues['distance']/StrToFloat(Qry.FieldValues['fuel_litre']),ffNumber,6,2)
          StrGrid.Cells[37,Count]:=FloatToStrF(Qry.FieldValues['distance']/TotalBBMLiter,ffNumber,6,2)
        else
          StrGrid.Cells[37,Count]:='0';
      end else begin
        StrGrid.Cells[35,Count]:='';
      ...
      *)
      ...
      if Qry.FieldValues['etoll_trx_date']<>NULL then StrGrid.Cells[41,Count]:=(Qry.FieldValues['etoll_trx_date']);
      if Qry.FieldValues['etoll_trx_time']<>NULL then StrGrid.Cells[42,Count]:=(Qry.FieldValues['etoll_trx_time']);

      if Qry.FieldValues['account_number']<>NULL then StrGrid.Cells[43,Count]:=Qry.FieldValues['account_number'];
      if Qry.FieldValues['account_name']<>NULL then StrGrid.Cells[44,Count]:=Qry.FieldValues['account_name'];
      if Qry.FieldValues['from_to_dates2']<>NULL then StrGrid.Cells[45,Count]:=Qry.FieldValues['from_to_dates2'];

      if Qry.FieldValues['jenisKendaraan']<>NULL then StrGrid.Cells[46,Count]:=Qry.FieldValues['jenisKendaraan'];
      if Qry.FieldValues['seat']<>NULL then StrGrid.Cells[47,Count]:=Qry.FieldValues['seat'];
      if Qry.FieldValues['JamSelesai']<>NULL then StrGrid.Cells[48,Count]:=Qry.FieldValues['JamSelesai'];
  ...
  for Count:=0 to 38 do begin
    ...
    StrGrid.CellStyle[39,StrGrid.RowCount-2].HorizontalAlignment:=taLeftJustify;
    StrGrid.CellStyle[40,StrGrid.RowCount-2].HorizontalAlignment:=taLeftJustify;
    StrGrid.CellStyle[41,StrGrid.RowCount-2].HorizontalAlignment:=taLeftJustify;
    StrGrid.CellStyle[42,StrGrid.RowCount-2].HorizontalAlignment:=taLeftJustify;

    StrGrid.CellStyle[43,StrGrid.RowCount-2].HorizontalAlignment:=taCenter;
    StrGrid.CellStyle[44,StrGrid.RowCount-2].HorizontalAlignment:=taLeftJustify;
    StrGrid.CellStyle[45,StrGrid.RowCount-2].HorizontalAlignment:=taLeftJustify;
    StrGrid.CellStyle[46,StrGrid.RowCount-2].HorizontalAlignment:=taLeftJustify;
...
begin
  if IntRow>0 then begin
    Case IntCol of
      ...
      3..13:if Main.IsFormOpen('OrderFee')=False then OrderFee:=TOrderFee.Create(Self,StrGrid.Cells[2,IntRow],False);
...
begin
  if Trim(edtCari.Text)<>'' then begin
    ...
    for Count:=0 to Length(OrderFeeArr)-1 do begin
      IsTrue:=False;
      for Count3:=0 to MaxCol-1 do
      if (StrPos(PChar(UpperCase(OrderFeeArr[Count][Count3])),PChar(UpperCase(edtCari.Text)))<>nil) then IsTrue:=True;
      if IsTrue then begin
          StrGrid.RowCount:=Count2+2;
          //StrGrid.Cells[0,Count2]:=IntToStr(Count-1);
          StrGrid.Cells[0,Count2]:=IToCurr(Count2-1);
          StrGrid.Cells[1,Count2]:=OrderFeeArr[Count][0];
          StrGrid.Cells[2,Count2]:=OrderFeeArr[Count][1];
          StrGrid.Cells[3,Count2]:=OrderFeeArr[Count][2];
          StrGrid.Cells[4,Count2]:=OrderFeeArr[Count][3];
          StrGrid.Cells[5,Count2]:=OrderFeeArr[Count][4];

          if (OrderFeeArr[Count][44]<>NULL) or (OrderFeeArr[Count][44]<>'') then begin  {employee_id}
            Inc(TotalOperasi);
            StrGrid.Cells[6,Count2]:=OrderFeeArr[Count][5];
            StrGrid.Cells[7,Count2]:=OrderFeeArr[Count][6];
            StrGrid.Cells[8,Count2]:=OrderFeeArr[Count][7];
            StrGrid.Cells[9,Count2]:=OrderFeeArr[Count][8];
            //Budget
            ...
            //SPBU
            if OrderFeeArr[Count][11]<>'0' then begin//StoInt(OrderFeeArr[Count][11])<>NULL then begin
            ...
            TotalTolParkir:=TotalTolParkir+StoInt(OrderFeeArr[Count][18]);
            TotalTol:=TotalTol+StoInt(OrderFeeArr[Count][19]);

            TotalFeeDriverReimburse:=TotalFeeDriverReimburse+(StoInt(OrderFeeArr[Count][20])); //*StoInt(OrderFeeArr[Count][15])
            TotalFeeBusBoyReimburse:=TotalFeeBusBoyReimburse+(StoInt(OrderFeeArr[Count][21])); //*StoInt(OrderFeeArr[Count][15])
            ...
            TotalLain:=TotalLain+StoInt(OrderFeeArr[Count][24]);
            TotalOvertime:=TotalOvertime+StoInt(OrderFeeArr[Count][25]);

            IntBiaya:=BBMRp+(StoInt(OrderFeeArr[Count][16])*StoInt(OrderFeeArr[Count][15]))+(StoInt(OrderFeeArr[Count][17])*StoInt(OrderFeeArr[Count][15]))+StoInt(OrderFeeArr[Count][18]);
            IntBiayaReimburse:=(StoInt(OrderFeeArr[Count][20])) + (StoInt(OrderFeeArr[Count][21])) + //*StoInt(OrderFeeArr[Count][15])
            ...
            TotalBiayaExc:=TotalBiayaExc+IntBiaya+IntBiayaReimburse+StoInt(OrderFeeArr[Count][24]);

            StrGrid.CellStyle[9,Count2].HorizontalAlignment:=taCenter;
            ...
            StrGrid.CellStyle[22,Count2].WordWrap:=True;
            ...
            StrGrid.CellStyle[24,Count2].WordWrap:=True;
            ...
            StrGrid.CellStyle[30,Count2].HorizontalAlignment:=taRightJustify;
            StrGrid.CellStyle[31,Count2].HorizontalAlignment:=taRightJustify;
            StrGrid.CellStyle[32,Count2].HorizontalAlignment:=taRightJustify;
            StrGrid.CellStyle[33,Count2].HorizontalAlignment:=taRightJustify;
            StrGrid.CellStyle[34,Count2].HorizontalAlignment:=taRightJustify;
            StrGrid.CellStyle[41,Count2].HorizontalAlignment:=taCenter;

            if (BBMLiterSingle_Budget=0) then StrGrid.Cells[10,Count2]:= '0'
            else StrGrid.Cells[10,Count2]:=SToCurr(FloatToStr(BBMLiterSingle_Budget),2);
            StrGrid.Cells[11,Count2]:=IToCurr(BBMRp_Budget);
            if (BBMLiterSingle_SPBU=0) then StrGrid.Cells[12,Count2]:= '0'
            else StrGrid.Cells[12,Count2]:=SToCurr(FloatToStr(BBMLiterSingle_SPBU),2);
            StrGrid.Cells[13,Count2]:=IToCurr(BBMRp_SPBU);
            if (BBMLiterSingle_Reimburse=0) then StrGrid.Cells[14,Count2]:= '0'
            else StrGrid.Cells[14,Count2]:=SToCurr(FloatToStr(BBMLiterSingle_Reimburse),2);
            StrGrid.Cells[15,Count2]:=IToCurr(BBMRp_Reimburse);

            if (BBMLiterSingle=0) then StrGrid.Cells[16,Count2]:= '0'
            else StrGrid.Cells[16,Count2]:=SToCurr(FloatToStr(BBMLiterSingle),2);
            StrGrid.Cells[17,Count2]:=IToCurr(BBMRp);

            {Geser 6 kolom}
            StrGrid.Cells[18,Count2]:=IToCurr(StoInt(OrderFeeArr[Count][16])*StoInt(OrderFeeArr[Count][15]));
            StrGrid.Cells[19,Count2]:=IToCurr(StoInt(OrderFeeArr[Count][20]));//*StoInt(OrderFeeArr[Count][15])

            StrGrid.Cells[20,Count2]:=IToCurr(StoInt(OrderFeeArr[Count][17])*StoInt(OrderFeeArr[Count][15]));
            StrGrid.Cells[21,Count2]:=IToCurr(StoInt(OrderFeeArr[Count][21]));//*StoInt(OrderFeeArr[Count][15])

            StrGrid.Cells[22,Count2]:=IToCurr(StoInt(OrderFeeArr[Count][18]));
            StrGrid.Cells[23,Count2]:=IToCurr(StoInt(OrderFeeArr[Count][22]));

            StrGrid.Cells[24,Count2]:=IToCurr(StoInt(OrderFeeArr[Count][19]));
            StrGrid.Cells[25,Count2]:=IToCurr(StoInt(OrderFeeArr[Count][23]));

            //if OrderFeeArr[Count][37]<>NULL then StrGrid.Cells[16,Count2]:='';//eToll(OrderFeeArr[Count][37]);
            StrGrid.Cells[26,Count2]:=IToCurr(StoInt(OrderFeeArr[Count][24]));
            StrGrid.Cells[27,Count2]:=IToCurr(StoInt(OrderFeeArr[Count][25]));
            StrGrid.Cells[28,Count2]:=IToCurr(IntBiaya+IntBiayaReimburse+StoInt(OrderFeeArr[Count][24])+StoInt(OrderFeeArr[Count][19]));
            StrGrid.Cells[29,Count2]:=IToCurr(IntBiaya+IntBiayaReimburse+StoInt(OrderFeeArr[Count][24]));
            StrGrid.Cells[30,Count2]:=OrderFeeArr[Count][26];

            if OrderFeeArr[Count][27]<>'' then begin
              StrGrid.CellStyle[30,Count2].Font.Color:=clBlue;
              StrGrid.CellStyle[31,Count2].Font.Color:=clBlue;
            end else begin
              StrGrid.CellStyle[30,Count2].Font.Color:=clBlack;
              StrGrid.CellStyle[31,Count2].Font.Color:=clBlack;
            end;
            if OrderFeeArr[Count][28]<>'' then StrGrid.Cells[31,Count2]:=OrderFeeArr[Count][28];

            if OrderFeeArr[Count][29]<>'' then begin
              StrGrid.CellStyle[32,Count2].Font.Color:=clBlue;
              StrGrid.CellStyle[33,Count2].Font.Color:=clBlue;
            end else begin
              StrGrid.CellStyle[32,Count2].Font.Color:=clBlack;
              StrGrid.CellStyle[33,Count2].Font.Color:=clBlack;
            end;
            if OrderFeeArr[Count][30]<>'' then StrGrid.Cells[32,Count2]:=OrderFeeArr[Count][30];
            if OrderFeeArr[Count][31]<>'' then StrGrid.Cells[33,Count2]:=OrderFeeArr[Count][31];
            if OrderFeeArr[Count][32]<>'' then StrGrid.Cells[34,Count2]:=IToCurr(StoInt(OrderFeeArr[Count][32]));
            if OrderFeeArr[Count][33]<>'' then begin
              StrGrid.Cells[35,Count2]:=IToCurr(SToInt(OrderFeeArr[Count][33]));
              StrGrid.Cells[36,Count2]:=IToCurr(StoInt(OrderFeeArr[Count][34]));
              if (SToFloat(OrderFeeArr[Count][9]))+(SToFloat(OrderFeeArr[Count][45]))+(SToFloat(OrderFeeArr[Count][11])) >0 then
                //StrGrid.Cells[32,Count2]:=FloatToStrF(StoInt(OrderFeeArr[Count][34])/StrToFloat(StoInt(OrderFeeArr[Count][45])),ffNumber,6,2)
                StrGrid.Cells[37,Count2]:=FloatToStrF(StoInt(OrderFeeArr[Count][34])/TotalBBMLiter,ffNumber,6,2)
              else
                StrGrid.Cells[37,Count2]:='0';
            end else begin
              StrGrid.Cells[35,Count2]:='';
              StrGrid.Cells[36,Count2]:='0';
              StrGrid.Cells[37,Count2]:='0';
            end;
            StrGrid.Cells[38,Count2]:=OrderFeeArr[Count][35];
            if (OrderFeeArr[Count][36])<>'' then StrGrid.Cells[39,Count2]:=(OrderFeeArr[Count][36]) else StrGrid.Cells[38,Count2]:='';
            if OrderFeeArr[Count][37]<>'' then StrGrid.Cells[40,Count2]:=eToll(OrderFeeArr[Count][37]);
            if OrderFeeArr[Count][38]<>'' then StrGrid.Cells[41,Count2]:=OrderFeeArr[Count][38];
            if OrderFeeArr[Count][39]>'' then StrGrid.Cells[42,Count2]:=OrderFeeArr[Count][39];
            if OrderFeeArr[Count][40]<>'' then StrGrid.Cells[43,Count2]:=OrderFeeArr[Count][40];

            if OrderFeeArr[Count][41]<>'' then StrGrid.Cells[44,Count2]:=OrderFeeArr[Count][41];
            if OrderFeeArr[Count][42]<>'' then StrGrid.Cells[45,Count2]:=OrderFeeArr[Count][42];
            if OrderFeeArr[Count][43]<>'' then StrGrid.Cells[46,Count2]:=OrderFeeArr[Count][43];
          end;
(*
          for Count4:=0 to MaxCol-1 do begin
            StrGrid.Cells[0,Count2]:=IToCurr(Count2);
            StrGrid.Cells[Count4+1,Count2]:=OrderFeeArr[Count][Count4];
