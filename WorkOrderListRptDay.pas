unit WorkOrderListRptDay;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, ComCtrls, StdCtrls, ADODB, WHUnit, Grids, ZColorStringGrid, Buttons;

type
  TWorkOrderRptDay = class(TForm)
    lbl2: TLabel;
    btnToXCel: TSpeedButton;
    ...
    procedure btnToXCelClick(Sender: TObject);
...
var
  WorkOrderRptDay: TWorkOrderRptDay;

implementation

uses
  MainU, WorkOrderFormInU, WorkOrderFormU, OrderFormU, MaintenanceServiceFormU,
  ServiceRequestFormU;

{$R *.dfm}

constructor TWorkOrderRptDay.Create(AOwner:TComponent;Form_Request:String='';Form_Function:String='');
begin
...
procedure TWorkOrderRptDay.Init;
begin
  SBU.Items.Clear;
  SBU.ItemIndex:=-1;
  SBU.Text:='';
...
procedure TWorkOrderRptDay.InitGrid;
...
procedure TWorkOrderRptDay.RefreshCombo;
var Qry:TADOQuery;
    StrQry:String;
    IntCount:Integer;
begin
...
procedure TWorkOrderRptDay.RefreshData;
var Qry:TADOQuery;
...
begin
  ...
  if Main.OpenDb then begin
    ...
    StrStatus:=',@Status=1';
    StrToDates:=',@ToDates='+QuotedStr(FormatDateTime('yyyy/mm/dd',Tanggal.date));
    StrQry:='EXEC GetWorkOrderLists '+StrCompanyId+','+StrLocationId+StrTanggal +StrStatus+',@StatusDay=''1'';';
    Qry.SQL.Add(StrQry);
    Qry.Open;
...
procedure TWorkOrderRptDay.RefreshGrid;
...
procedure TWorkOrderRptDay.FormClose(Sender: TObject;
...
procedure TWorkOrderRptDay.FormShow(Sender: TObject);
begin
  Init;
  InitGrid;
  Initiation:=False;
...
procedure TWorkOrderRptDay.btnSelesaiClick(Sender: TObject);
begin
  Close;
end;

procedure TWorkOrderRptDay.LihatClick(Sender: TObject);
begin
...
procedure TWorkOrderRptDay.Search;
...
//        end else
        if (trim(Cari.Text)<>'') then begin
          if (StrPos(PChar(UpperCase(WorkOrderArr[IntCount][11])),PChar(UpperCase(Cari.Text)))<>nil) then IsTrue:=True;
        end else begin
...
procedure TWorkOrderRptDay.btnTombolCariClick(Sender: TObject);
begin
  Search;
end;

procedure TWorkOrderRptDay.btnToXCelClick(Sender: TObject);
begin
  if ToExcel4(StrGrid) then ShowMessage('Export ke Excel Berhasil');
end;

procedure TWorkOrderRptDay.StrGridDblClick(Sender: TObject);
begin
...
procedure TWorkOrderRptDay.StrGridSelectCell(Sender: TObject; ACol,
