unit CustomerComplainCorrectionListU;
...
type
  TCustomerComplainCorrectionList = class(TForm)
    Label2: TLabel;
...
var
  CustomerComplainCorrectionList: TCustomerComplainCorrectionList;

implementation

uses MainU, CustomerComplainCorrection, CustomerComplainCorrectiveActionPlanU;

{$R *.dfm}
constructor TCustomerComplainCorrectionList.Create(AOwner:TComponent;Form_Request:String='';Customer_Id:String='';Tipe_Data:Integer=0;Is_Closed:Byte=0);
begin
...
procedure TCustomerComplainCorrectionList.Init;
begin
  Cari.Text:='';
end;

procedure TCustomerComplainCorrectionList.InitGrid;
...
begin
  ...
  GridCCP.ColWidths[21]:=0;

  GridCCP.SizingHeight:=True;

  GridCCP.Cells[0,0]:='Tanggal Perbaikan';
...
procedure TCustomerComplainCorrectionList.RefreshCombo;
var Qry:TADOQuery;
    StrQry:String;
    IntCount:Integer;
begin
...
procedure TCustomerComplainCorrectionList.RefreshData;
...
begin
  ...
  if Main.OpenDb then begin
    ...
    if (Qry.RecordCount>0) then while not(Qry.Eof) do begin
       ...
       if Qry.FieldValues['handling_date']<>NULL then
        CCPArr[IntCount][0]:=Qry.FieldValues['handling_date']
...
procedure TCustomerComplainCorrectionList.RefreshGrid;
var IntCount:Integer;
begin
...
procedure TCustomerComplainCorrectionList.Show;
begin
...
procedure TCustomerComplainCorrectionList.SelesaiClick(Sender: TObject);
begin
  Close;
end;

procedure TCustomerComplainCorrectionList.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TCustomerComplainCorrectionList.FormShow(Sender: TObject);
begin
  InitGrid;
  RefreshCombo;
  Show;
...
procedure TCustomerComplainCorrectionList.GridCCPDblClick(Sender: TObject);
begin
  if (IntRow>MinRowGrid) and (GridCCP.Cells[0,IntRow]<>'') then begin
    if UpperCase(FormRequest)='MAIN-VIEW' then  begin
      if Main.IsFormOpen('CustomerComplainCorrectionForm')=False then
        CustomerComplainCorrectionForm:=TCustomerComplainCorrectionForm.Create(Self,GridCCP.Cells[1,IntRow],'VIEW');
    end else if UpperCase(FormRequest)='MAIN-UPDATE' then  begin
      if Main.IsFormOpen('CustomerComplainCorrectionForm')=False then
      begin
        CustomerComplainCorrectionForm:=TCustomerComplainCorrectionForm.Create(Self,GridCCP.Cells[1,IntRow],'UPDATE');
        CustomerComplainInvestigationID:=StrToInt(GridCCP.Cells[21,IntRow]);
      end;
    end else if UpperCase(FormRequest)='CREATE-CORRECTIVE' then  begin
      if Main.IsFormOpen('CustComplainCorrectiveActionPlan')=True then
      begin
        CustComplainCorrectiveActionPlan.SetData2(GridCCP.Cells[1,IntRow]);
        Close;
      end;
    end;
  end;
...
procedure TCustomerComplainCorrectionList.GridCCPSelectCell(
  Sender: TObject; ACol, ARow: Integer; var CanSelect: Boolean);
begin
  IntRow:=ARow;
end;

procedure TCustomerComplainCorrectionList.LihatClick(Sender: TObject);
begin
  RefreshData;
  RefreshGrid;
end;

procedure TCustomerComplainCorrectionList.CariChange(Sender: TObject);
