object IntegrateGPHPPBus: TIntegrateGPHPPBus
  Left = 107
  Top = 179
  BorderIcons = []
  BorderStyle = bsNone
  Caption = 'Integrate HPP Bus'
  ClientHeight = 398
  ClientWidth = 1169
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object ToXCel1: TSpeedButton
    Left = 1126
    Top = 12
    Width = 23
    Height = 22
    BiDiMode = bdLeftToRight
    Glyph.Data = {
      
      E2C5C3E2C5C2E2C2BFE0BFBDDFBDBCDFBCBDDFBDB3D8B286AD80}
    Layout = blGlyphRight
    ParentBiDiMode = False
    OnClick = ToXCel1Click
  end
  object Label2: TLabel
    Left = 882
    Top = 15
    Width = 61
    Height = 13
    Alignment = taRightJustify
    Caption = 'Dari Tanggal'
  end
  object Label3: TLabel
    Left = 886
    Top = 39
    Width = 58
    Height = 13
    Alignment = taRightJustify
    Caption = 's/d Tanggal'
    Visible = False
  end
  object Selesai: TButton
    Left = 1075
    Top = 365
    Width = 75
    Height = 25
    Caption = 'Selesai'
    TabOrder = 0
    OnClick = SelesaiClick
    OnKeyDown = SelesaiKeyDown
  end
  object FromDates: TDateTimePicker
    Left = 948
    Top = 13
    Width = 89
    Height = 21
    Date = 41198.000000000000000000
    Time = 41198.000000000000000000
    TabOrder = 1
  end
  object StrGrid1: TZColorStringGrid
    Left = 8
    Top = 68
    Width = 567
    Height = 145
    ColCount = 9
    DefaultRowHeight = 20
    DefaultDrawing = False
    FixedColor = clBtnFace
    FixedCols = 0
    FixedRows = 2
    Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goRangeSelect, goDrawFocusSelected, goColSizing]
    TabOrder = 3
    DefaultCellStyle.Font.Charset = DEFAULT_CHARSET
    DefaultCellStyle.Font.Color = clWindowText
    DefaultCellStyle.Font.Height = -11
    DefaultCellStyle.Font.Name = 'MS Sans Serif'
    DefaultCellStyle.Font.Style = []
    DefaultCellStyle.BGColor = clWindow
    DefaultFixedCellStyle.Font.Charset = DEFAULT_CHARSET
    DefaultFixedCellStyle.Font.Color = clWindowText
    DefaultFixedCellStyle.Font.Height = -11
    DefaultFixedCellStyle.Font.Name = 'MS Sans Serif'
    DefaultFixedCellStyle.Font.Style = []
    DefaultFixedCellStyle.BGColor = clBtnFace
    LineDesign.LineUpColor = clWhite
  end
  object Process: TButton
    Left = 1046
    Top = 11
    Width = 75
    Height = 23
    Caption = 'Lihat Data'
    TabOrder = 4
    OnClick = ProcessClick
  end
  object GroupLokasi: TGroupBox
    Left = 239
    Top = 0
    Width = 221
    Height = 33
    Enabled = False
    TabOrder = 2
    object Label1: TLabel
      Left = 5
      Top = 12
      Width = 31
      Height = 13
      Alignment = taRightJustify
      Caption = 'Lokasi'
    end
    object LokasiDisp: TEdit
      Left = 91
      Top = 9
      Width = 124
      Height = 21
      TabOrder = 0
    end
    object Lokasi: TComboBox
      Left = 40
      Top = 9
      Width = 49
      Height = 21
      ItemHeight = 13
      TabOrder = 1
      OnChange = LokasiChange
    end
  end
  object StrGrid2: TZColorStringGrid
    Left = 582
    Top = 68
    Width = 567
    Height = 145
    ColCount = 9
    DefaultRowHeight = 20
    DefaultDrawing = False
    FixedColor = clBtnFace
    FixedCols = 0
    FixedRows = 2
    Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goRangeSelect, goDrawFocusSelected, goColSizing]
    TabOrder = 5
    DefaultCellStyle.Font.Charset = DEFAULT_CHARSET
    DefaultCellStyle.Font.Color = clWindowText
    DefaultCellStyle.Font.Height = -11
    DefaultCellStyle.Font.Name = 'MS Sans Serif'
    DefaultCellStyle.Font.Style = []
    DefaultCellStyle.BGColor = clWindow
    DefaultFixedCellStyle.Font.Charset = DEFAULT_CHARSET
    DefaultFixedCellStyle.Font.Color = clWindowText
    DefaultFixedCellStyle.Font.Height = -11
    DefaultFixedCellStyle.Font.Name = 'MS Sans Serif'
    DefaultFixedCellStyle.Font.Style = []
    DefaultFixedCellStyle.BGColor = clBtnFace
    LineDesign.LineUpColor = clWhite
  end
  object ToDates: TDateTimePicker
    Left = 948
    Top = 37
    Width = 89
    Height = 21
    Date = 41198.000000000000000000
    Time = 41198.000000000000000000
    TabOrder = 6
    Visible = False
  end
  object ToDate: TCheckBox
    Left = 870
    Top = 37
    Width = 17
    Height = 17
    Caption = 'ToDate'
    TabOrder = 7
    Visible = False
    OnClick = ToDateClick
  end
  object StrGrid3: TZColorStringGrid
    Left = 8
    Top = 217
    Width = 567
    Height = 145
    ColCount = 9
    DefaultRowHeight = 20
    DefaultDrawing = False
    FixedColor = clBtnFace
    FixedCols = 0
    FixedRows = 2
    Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goRangeSelect, goDrawFocusSelected, goColSizing]
    TabOrder = 8
    DefaultCellStyle.Font.Charset = DEFAULT_CHARSET
    DefaultCellStyle.Font.Color = clWindowText
    DefaultCellStyle.Font.Height = -11
    DefaultCellStyle.Font.Name = 'MS Sans Serif'
    DefaultCellStyle.Font.Style = []
    DefaultCellStyle.BGColor = clWindow
    DefaultFixedCellStyle.Font.Charset = DEFAULT_CHARSET
    DefaultFixedCellStyle.Font.Color = clWindowText
    DefaultFixedCellStyle.Font.Height = -11
    DefaultFixedCellStyle.Font.Name = 'MS Sans Serif'
    DefaultFixedCellStyle.Font.Style = []
    DefaultFixedCellStyle.BGColor = clBtnFace
    LineDesign.LineUpColor = clWhite
  end
  object StrGrid4: TZColorStringGrid
    Left = 582
    Top = 217
    Width = 567
    Height = 145
    ColCount = 9
    DefaultRowHeight = 20
    DefaultDrawing = False
    FixedColor = clBtnFace
    FixedCols = 0
    FixedRows = 2
    Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goRangeSelect, goDrawFocusSelected, goColSizing]
    TabOrder = 9
    DefaultCellStyle.Font.Charset = DEFAULT_CHARSET
    DefaultCellStyle.Font.Color = clWindowText
    DefaultCellStyle.Font.Height = -11
    DefaultCellStyle.Font.Name = 'MS Sans Serif'
    DefaultCellStyle.Font.Style = []
    DefaultCellStyle.BGColor = clWindow
    DefaultFixedCellStyle.Font.Charset = DEFAULT_CHARSET
    DefaultFixedCellStyle.Font.Color = clWindowText
    DefaultFixedCellStyle.Font.Height = -11
    DefaultFixedCellStyle.Font.Name = 'MS Sans Serif'
    DefaultFixedCellStyle.Font.Style = []
    DefaultFixedCellStyle.BGColor = clBtnFace
    LineDesign.LineUpColor = clWhite
  end
  object GroupCompany: TGroupBox
    Left = 8
    Top = 0
    Width = 227
    Height = 33
    TabOrder = 10
    object Label11: TLabel
      Left = 8
      Top = 11
      Width = 22
      Height = 13
      Alignment = taRightJustify
      Caption = 'SBU'
    end
    object SBU: TComboBox
      Left = 34
      Top = 8
      Width = 185
      Height = 21
      Style = csDropDownList
      ItemHeight = 13
      TabOrder = 0
    end
  end
  object rbdk: TRadioButton
    Left = 16
    Top = 40
    Width = 82
    Height = 17
    Caption = 'Dalam kota'
    TabOrder = 11
  end
  object rblk: TRadioButton
    Left = 104
    Top = 40
    Width = 87
    Height = 17
    Caption = 'Luar kota'
    TabOrder = 12
  end
  object grpPilihanBiaya: TGroupBox
    Left = 463
    Top = 0
    Width = 370
    Height = 65
    Caption = '  Pilihan Biaya  '
    TabOrder = 13
    object rbBiaya1: TRadioButton
      Left = 8
      Top = 14
      Width = 321
      Height = 17
      Caption = 'Budget (Fee Driver, Fee Helper, BBM, Parkir dan Lain2)'
      Checked = True
      TabOrder = 0
      TabStop = True
    end
    object rbBiaya2: TRadioButton
      Left = 8
      Top = 30
      Width = 281
      Height = 17
      Caption = 'Reimburse (Fee Driver, Fee Helper, BBM, Tol dan Parkir)'
      TabOrder = 1
    end
    object rbBiaya3: TRadioButton
      Left = 8
      Top = 46
      Width = 113
      Height = 17
      Caption = 'BBM SPBU'
      TabOrder = 2
    end
    object rbBiaya4: TRadioButton
      Left = 216
      Top = 46
      Width = 145
      Height = 17
      Caption = 'eTol'
      TabOrder = 3
    end
  end
  object posting: TButton
    Left = 758
    Top = 28
    Width = 75
    Height = 22
    Caption = 'Integrate'
    TabOrder = 14
    Visible = False
    OnClick = postingClick
  end
  object Memo1: TMemo
    Left = 24
    Top = 248
    Width = 521
    Height = 81
    Lines.Strings = (
      'Memo1')
    ScrollBars = ssBoth
    TabOrder = 15
    Visible = False
  end
  object butIntegrate: TButton
    Left = 1046
    Top = 35
    Width = 75
    Height = 26
    Caption = 'Integrate All'
    TabOrder = 16
    OnClick = butIntegrateClick
  end
end
