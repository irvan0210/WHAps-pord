...
type
  TBookingForm = class(TForm)
    ...
    procedure GeserDriver2Click(Sender: TObject);
  private
...
begin
  ...
  StrGrid.Cells[9,0]:='Driver 1';
  StrGrid.Cells[10,0]:='Driver 2';
...
begin
  ...
  if IsOk then begin
    ...
    StrGrid.Cells[13,IntFreeRow]:=StrProductPrice;
    StrGrid.Cells[14,IntFreeRow]:=Order_Detail_Id;
    StrGrid.Cells[15,IntFreeRow]:=StrVhcBatchId;
    StrGrid.Cells[18,IntFreeRow]:=StrSeat;
    StrGrid.Cells[20,IntFreeRow]:=StrFullDay;
    StrGrid.Cells[21,IntFreeRow]:=StrHour;
    if Str_Reason<>'' then StrGrid.Cells[23,IntFreeRow]:=Str_Reason;
  end else
      ...
      if Main.OpenDb then begin
        ...
        if IsMoveUnit Then begin
          for IntCount:=MinRowGrid+1 to StrGrid.RowCount-1 do begin
            if (StrGrid.Cells[22,IntCount]='') and (StrGrid.Cells[0,IntCount]<>'')  then begin
                if Qry.RecordCount>0 then begin
                  if StrBodyIdAsal=StrGrid.Cells[8,IntCount] then begin
                    ...
                    if Qry.FieldValues['busboy_name']<>NULL then begin
                      StrGrid.Cells[11,IntCount]:=Qry.FieldValues['busboy_name'];
                      StrGrid.Cells[33,IntCount]:=Qry.FieldValues['busboy_id'];
                      StrGrid.Cells[34,IntCount]:=Qry.FieldValues['busboy_cellular_no'];
                    end;
          ...
          if Qry.RecordCount>0 then begin
            ...
            if Qry.FieldValues['busboy_name']<>NULL then begin
              StrGrid.Cells[11,IntRow]:=Qry.FieldValues['busboy_name'];
              StrGrid.Cells[33,IntRow]:=Qry.FieldValues['busboy_id'];
              StrGrid.Cells[34,IntRow]:=Qry.FieldValues['busboy_cellular_no'];
            end;
          ...
          for IntCount:=MinRowGrid+1 to StrGrid.RowCount-1 do begin
            if Qry.RecordCount>0 then begin
              if Is_First then begin
                if (StrGrid.Cells[22,IntCount] = null) or (StrGrid.Cells[22,IntCount] = '') then
                begin
                  StrGrid.Cells[9,IntCount]:=Qry.FieldValues['name'];
                  StrGrid.Cells[17,IntCount]:=Qry.FieldValues['employee_id'];    //sudah diganti
                  StrGrid.Cells[25,IntCount]:=Qry.FieldValues['name']; {driver name}   //sudah diganti
                ...
                begin
                  ...
                  StrGrid.Cells[43,IntRow]:=Qry.FieldValues['cellular_no'];
                end;
              end;
            end;
          end;
...
var Qry:TADOQuery;
    StrQry,helpername,StrEMessage:String;
    IntCount:Integer;
    IsFree,IsFound:Boolean;
  ...
  if Employee_Id<>'' then begin
    ...
    if IsFree=True then begin
      ...
      if Main.OpenDb then begin
        ...
        begin
          MessageBox(Handle,PChar('Helper '+QuotedStr(Qry.FieldValues['name'])+Chr(13)+Chr(13)+'Tidak ada no HP'),'Geser Helper',MB_OK or MB_ICONERROR or MB_SYSTEMMODAL or MB_SETFOREGROUND);
        end else begin
            ...
            for IntCount:=MinRowGrid+1 to StrGrid.RowCount-1 do begin
              if Qry.RecordCount>0 then begin
                ...
                begin
                  ...
                  StrGrid.Cells[34,IntCount]:=Qry.FieldValues['cellular_no'];;   //sudah diganti
                end;
              end;
...
//    IsCanCancel:=True;
    if Qry.RecordCount>0 then while not(Qry.Eof) do begin
     ...
     // else StrGrid.Cells[11,IntCount]:='Tes Kenek';
      ...
      StrGrid.Cells[16,IntCount]:=Qry.FieldValues['vehicle_id'];    //sudah diganti
      StrGrid.Cells[17,IntCount]:=Qry.FieldValues['employee_id'];    //Driver Terbaru
...
//        IsCanCancel:=False;
      ...
      StrGrid.Cells[31,IntCount]:=Qry.FieldValues['employee_id'];  //Driver Awal
      StrGrid.Cells[32,IntCount]:=Qry.FieldValues['vehicle_id']; //sudah diganti

      if (Qry.FieldValues['busboy_id']<>NULL) then StrGrid.Cells[33,IntCount]:=Qry.FieldValues['busboy_id'] else StrGrid.Cells[33,IntCount]:='';  //busboy ganti
      ...
      if (Qry.FieldValues['driver_status']<>NULL) then StrGrid.Cells[35,IntCount]:=Qry.FieldValues['driver_status'];
      if (Qry.FieldValues['helper_status']<>NULL) then StrGrid.Cells[36,IntCount]:=Qry.FieldValues['helper_status'];
      if (Qry.FieldValues['busboy_id']<>NULL) then StrGrid.Cells[37,IntCount]:=Qry.FieldValues['busboy_id'] else StrGrid.Cells[37,IntCount]:='';
      if (Qry.FieldValues['driver_confirm_date']<>NULL) then StrGrid.Cells[38,IntCount]:=Qry.FieldValues['driver_confirm_date'];
      if (Qry.FieldValues['helper_confirm_date']<>NULL) then StrGrid.Cells[39,IntCount]:=Qry.FieldValues['helper_confirm_date'];
      if (Qry.FieldValues['driver2_status']<>NULL) then StrGrid.Cells[40,IntCount]:=Qry.FieldValues['driver2_status'];
      if (Qry.FieldValues['driver2_confirm_date']<>NULL) then StrGrid.Cells[41,IntCount]:=Qry.FieldValues['driver2_confirm_date'];
      if Qry.FieldValues['employee_id2']<>NULL then StrGrid.Cells[42,IntCount]:=Qry.FieldValues['employee_id2'];    //sudah diganti
      if (Qry.FieldValues['driver2_cellphone']<>NULL) then StrGrid.Cells[43,IntCount]:=Qry.FieldValues['driver2_cellphone'] else StrGrid.Cells[43,IntCount]:='';
      if (Qry.FieldValues['helper']=1) then StrGrid.Cells[44,IntCount]:='1' else StrGrid.Cells[44,IntCount]:='0';
      Qry.Next;
      Inc(IntCount)
    end;
    Qry.Close;
...
begin
  if (OrderId.Text<>'') and (CustomerId.Text<>'')  then begin
      ...
      for IntCount:=1 to StrGrid.RowCount-1 do begin
        if (StrGrid.Cells[44,IntCount]='1') and (StrGrid.Cells[11,IntCount]='') and (StrGrid.Cells[22,IntCount]='') then
        begin
          MessageBox(Handle,PChar('Data Gagal Disimpan, Unit Big Bus Wajib Pakai Helper'+Chr(13)+Chr(13)+StrEMsg),'Penjadwalan',MB_OK or MB_ICONERROR or MB_SYSTEMMODAL or MB_SETFOREGROUND);
          IsOk:=False;
          Main.M_Normal;
          IsInputGrid:=True;
          IsInput:=True;
          ...
          if not(IsSetLock) then begin
            ...
            if StrStatus='1' then begin
              ...
              for IntCount:=2 to StrGrid.RowCount-1 do begin
                ...
                and (StrGrid.Cells[16,IntCount]<>'') and (StrGrid.Cells[17,IntCount]<>'') then begin   //sudah diganti
                  Inc(IntCount2);
                  StrQry := '';
...
//                  if StrPackage<>'1' then begin
                  ...
                  if StrGrid.Cells[10,IntCount]<>'' then StrDriver2Name:=QuotedStr(StrGrid.Cells[10,IntCount]) else StrDriver2Name:='NULL';
                  if StrGrid.Cells[43,IntCount]<>'' then StrDriver2Phone:=QuotedStr(StrGrid.Cells[43,IntCount]) else StrDriver2Phone:='NULL';
...
//                  end;
                  ...
                  StrEmployeeIdLast:=QuotedStr(StrGrid.Cells[31,IntCount]);

                  if (Trim(StrEmployeeId)<>Trim(StrEmployeeIdLast)) then
                  begin
                    StrDriverStatus:=QuotedStr('WAITING');
                    StrDriverConfirmDate:='NULL';
                  end else
                  begin
                    StrDriverStatus:=QuotedStr(StrGrid.Cells[35,IntCount]);
                    if StrGrid.Cells[38,IntCount]='' then StrDriverConfirmDate:='NULL' else
//                    StrDriverConfirmDate:=QuotedStr(FormatDateTime('yyyy-mm-dd hh:nn:dd',StrToDate(StrGrid.Cells[38,IntCount])));
                  end;

                  if (StremployeeId3<>QuotedStr(StrGrid.Cells[37,IntCount])) AND (StrGrid.Cells[33,IntCount]<>'') then
                  begin
                    StrHelperStatus:=QuotedStr('WAITING');
                    StrHelperConfirmDate:='NULL';
                  end else if (StremployeeId3=QuotedStr(StrGrid.Cells[37,IntCount])) then
                  begin
                    StrHelperStatus:=QuotedStr(StrGrid.Cells[36,IntCount]);
                    if StrGrid.Cells[39,IntCount]='' then StrHelperConfirmDate:='NULL' else
//                    StrHelperConfirmDate:=QuotedStr(FormatDateTime('yyyy-mm-dd hh:nn:dd',StrToDate(StrGrid.Cells[39,IntCount])));
                  end else  begin
                    StrHelperStatus:='NULL';
                    StrHelperConfirmDate:='NULL';
                  end;

                  if (StremployeeId2<>QuotedStr(StrGrid.Cells[42,IntCount])) AND (StrGrid.Cells[19,IntCount]<>'') then
                  begin
                    StrDriver2Status:=QuotedStr('WAITING');
                    StrDriver2ConfirmDate:='NULL';
                  end else if (StremployeeId2=QuotedStr(StrGrid.Cells[42,IntCount])) then
                  begin
                    StrDriver2Status:=QuotedStr(StrGrid.Cells[40,IntCount]);
                    if StrGrid.Cells[41,IntCount]='' then StrDriver2ConfirmDate:='NULL' else
//                    StrDriver2ConfirmDate:=QuotedStr(FormatDateTime('yyyy-mm-dd hh:nn:dd',StrToDate(StrGrid.Cells[41,IntCount])));
                  end else  begin
                    StrDriver2Status:='NULL';
                    StrDriver2ConfirmDate:='NULL';
                  end;
  ...
  //                isAuth, isAuthReason,
                  if (StrVhcTransId='NULL') then begin
                    StrQry:='INSERT INTO wh_reserved_order_detail (reserved_order_detail_id,reserved_order_id'+
                            ...
                            ' isAuthReason,employee_id3,driver_status,helper_status,driver2_status ) VALUES '+
                            ...
                            ','+StrIsAuth+','+StrIsAuthReason+','+StremployeeId3+','+StrDriverStatus+','+StrHelperStatus+','+StrDriver2Status+');';
  ...
  //                GESER UNIT
                  ...
                  //  if (StrVhcTransId='NULL') then begin
                      ...
                      StrQryWehaOnlineCek:='SELECT WehaReservedCode From OrderDetailVehicleInfos'+
                                          ' WHERE WehaReservedCode='+QuotedStr(StrGrid.Cells[24,IntCount]);
                      QryWehaOnline.Close;
                      QryWehaOnline.SQL.Clear;
                      QryWehaOnline.SQL.Add(StrQryWehaOnlineCek);
                      QryWehaOnline.Open;
                      ...
                      begin
                        ...
                        begin
                          StrConfirmHelperVehicleInfos:=' ,HelperConfirmedByAppVersion=NULL,HelperConfirmedDate=NULL ';
                        end else begin
                        ...
                        begin
                          StrConfirmDriver2VehicleInfos:=' ,DriverBackupConfirmedByAppVersion=NULL,DriverBackupConfirmedDate=NULL ';
                        end else begin
...
//                        HELPER
                        ...
                        begin
                          ...
                          end else if QryWehaOnline2.RecordCount=0 then begin
                            StrQryWehaOnlineUser:= 'INSERT INTO Contacts '+
                                          '(FullName,Gender,'+
                                          'HP,ViewHisOwnData,IsMain,CreatedBy,CreatedDate,'+
                                          'ModifiedBy,ModifiedDate,ViewGroupOnly) VALUES '+
...
//                        DRIVER 2
                        ...
                        begin
                          ...
                          StrQryWehaOnlineCek:='SELECT b.UserID,a.FullName,a.HP FROM Contacts a '+
                                                 'left join Users b ON a.ContactID=b.ContactID WHERE '+
                                                 'b.CustomerNo='+StremployeeId2+' AND b.IsActive=1';
                          QryWehaOnline2.Close;
                          QryWehaOnline2.SQL.Clear;
                          QryWehaOnline2.SQL.Add(StrQryWehaOnlineCek);
                          QryWehaOnline2.Open;
                          ...
                          if QryWehaOnline2.RecordCount>0 then begin
                            ...
                            try
                              ...
                              on E:Exception do begin
                                ...
                                EnableInput;
                                StrEMsg:=StrEMsg+E.Message;
                                MessageBox(Handle,PChar('Penjadwalan tidak bisa diinput '+Chr(13)+Chr(13)+StrEMsg),'Penjadwalan',MB_OK or MB_ICONERROR or MB_SYSTEMMODAL or MB_SETFOREGROUND);
                                Exit;
                              end;
                            end;
...
//                                ' WHERE WehaReservedCode='+StrReservedOrderDetailId;
                        ...
                        if StrHelperFullName = '' then StrHelperFullName := 'NULL';
                        if StrHelperHP = '' then StrHelperHP :=  'NULL';
                        if StrHelperUserID = '' then StrHelperUserID :=  'NULL';

                        if StrDriver2FullName = '' then StrDriver2FullName := 'NULL';
                        if StrDriver2HP = '' then StrDriver2HP :=  'NULL';
                        if StrDriver2UserID = '' then StrDriver2UserID :=  'NULL';
                        ...
                        StrQryWehaOnline:=  StrQryWehaOnline+' UPDATE OrderDetailVehicleInfos SET '+
                                            ...
                                            ' DriverBackupCustomerNo='+StremployeeId2+',DriverBackupID='+StrDriver2UserID+' '+
                                            ' '+StrConfirmDriverVehicleInfos+StrConfirmHelperVehicleInfos+StrConfirmDriver2VehicleInfos+' '+
                      ...
                      begin
                        ...
                        EnableInput;
                        Main.M_Normal;
                        StrEMsg:=QuotedStr(StrGrid.Cells[24,IntCount])+' not found';
                        MessageBox(Handle,PChar('Data Gagal Disimpan'+Chr(13)+Chr(13)+StrEMsg),'Penjadwalan',MB_OK or MB_ICONERROR or MB_SYSTEMMODAL or MB_SETFOREGROUND);
                        Exit;
                      end;
                   // end;
...
//                  HELPER
                    ...
                    StrQryWehaOnline:=  StrQryWehaOnline+' INSERT INTO '+
                                        ...
                                        ',ModifiedDate,WEHACustomerNo,'+
                                        'HelperName,HelperPhone,WEHAHelperCustomerNo,HelperID,'+
                                        'DriverBackupName,DriverBackupPhone,DriverBackupCustomerNo,DriverBackupID) VALUES '+
                                        ...
                                        ','+StrHelperFullName+','+StrHelperHP+','+StremployeeId3+','+StrHelperUserID+' '+
                                        ','+StrDriver2FullName+','+StrDriver2HP+','+StremployeeId2+','+StrDriver2UserID+');';
                  end;
                  ...
                  StrVehicleOld:=StrGrid.Cells[32,IntCount]; //sudah diganti
...
//                  end;
  ...
  //            begin
              ...
              if LeftStr(UpperCase(FormRequest),11)='WAITINGLIST' then  //sudah diganti
              begin
                if Status.Checked=True then
                begin
            ...
            if (IsOk) then begin
              ...
              if LeftStr(UpperCase(FormRequest),11)='WAITINGLIST' then begin  //sudah diganti
...
//    end else begin
      ...
      ListKunciCepat.Lines.Add('Delete = Menghapus Penjadwalan');
...
procedure TBookingForm.DeleteDriver2;
var IntCount:Integer;
begin
  if IntRow>MinRowGrid then begin
    if  StrGrid.Cells[10,IntRow] <>'' then begin
      if (StrGrid.Cells[22,IntRow] = null) or (StrGrid.Cells[22,IntRow] = '') then begin
        if  MessageBox(0,PChar('Driver 2 '+StrGrid.Cells[10,IntRow]+' Mau Dihapus ?') ,'Helper',MB_OKCANCEL or MB_ICONINFORMATION)=1 then begin
          if IntCol=10 then begin
            StrGrid.Cells[10,IntRow]:='';  //sudah diganti
            StrGrid.Cells[19,IntRow]:='';    //sudah Ada
          end else begin
            StrGrid.Cells[10,IntRow]:='';     //sudah diganti
            StrGrid.Cells[19,IntRow]:='';     //sudah Ada   'Helper tidak bisa dirubah'+Chr(13)+Chr(13)+'Sudah ada surat jalan'
          end;
        end;
      end else
     MessageBox(Handle,PChar('Dirver 2 tidak bisa dihapus'+Chr(13)+'Sudah ada surat jalan'),'Hapus Driver 2',MB_OK or MB_ICONERROR or MB_SYSTEMMODAL or MB_SETFOREGROUND);
    end else
    MessageBox(Handle,PChar('Dirver 2'),'Hapus Helper',MB_OK or MB_ICONWARNING or MB_SYSTEMMODAL or MB_SETFOREGROUND);
  end;
end;
...
begin
  if (IsInput) and (LeftStr(UpperCase(FormRequest),11)<>'WAITINGLIST') then begin
    ...
    if Key=VK_F5 then begin
      ...
      if (AuthorizedForm.ShowModal<>1) then IsAuth:=False;
      if IsAuth then VehicleRDList:=TVehicleRDList.Create(Self,'Bus','','','Reserved-Create',FormatDateTime('yyyy/mm/dd',StrToDate(StrGrid.Cells[4,IntRow])),FormatDateTime('yyyy/mm/dd',StrToDate(StrGrid.Cells[5,IntRow])),StrGrid.Cells[6,IntRow],True, False, StrGrid.Cells[15,IntRow] ); //sudah tidak ada
...
//    if (IntRow > MinRowGrid) and (StrGrid.Cells[21,IntRow]='') and (IsAuth=True)  then begin
  ...
  if (IntRow > MinRowGrid) and (StrGrid.Cells[22,IntRow]='') then begin   //sudah diganti
    Case IntCol Of
      0..4:begin
            GetOrderDetail;
      ...
      8:begin
          if (Main.IsFormOpen('VehicleList')=False) and (StrGrid.Cells[0,IntRow]<>'') then
            if LeftStr(UpperCase(FormRequest),11)='WAITINGLIST' then begin
              ...
              VehicleRDList:=TVehicleRDList.Create(Self,'Bus',StrGrid.Cells[15,IntRow],StrGrid.Cells[18,IntRow],'Reserved-Create',FormatDateTime('yyyy/mm/dd',StrToDate(StrGrid.Cells[4,IntRow])),FormatDateTime('yyyy/mm/dd',StrToDate(StrGrid.Cells[5,IntRow])),StrGrid.Cells[6,IntRow] );  //sudah diganti
            end;
        end;
      9:begin
...
//              if GridService.Cells[5,IntCount]='150201' then IsDoubleDriver:=True;
          if (Main.IsFormOpen('EmployeeList')=False) and (StrGrid.Cells[0,IntRow]<>'') and (hari>0) then
            EmployeeRDList:=TEmployeeRDList.Create(Self,'Bus',1,0,'Reserved-Create-CoDriver',FormatDateTime('yyyy/mm/dd',StrToDate(StrGrid.Cells[4,IntRow])),FormatDateTime('yyyy/mm/dd',StrToDate(StrGrid.Cells[5,IntRow])),False,IntRow);
        end;
        11:begin
          if (StrGrid.Cells[22,IntRow] = null) or (StrGrid.Cells[22,IntRow] = '') then begin
            if (Main.IsFormOpen('EmployeeList')=False) and (StrGrid.Cells[0,IntRow]<>'') then
            if LeftStr(UpperCase(FormRequest),11)='WAITINGLIST' then begin    //sudah diganti
              EmployeeRDList:=TEmployeeRDList.Create(Self,'Bus2',1,0,'WaitingList-Allocate',FormatDateTime('yyyy/mm/dd',StrToDate(StrGrid.Cells[4,IntRow])),FormatDateTime('yyyy/mm/dd',StrToDate(StrGrid.Cells[5,IntRow])));
            end else begin
              EmployeeRDList:=TEmployeeRDList.Create(Self,'Bus2',1,0,'Reserved-Create',FormatDateTime('yyyy/mm/dd',StrToDate(StrGrid.Cells[4,IntRow])),FormatDateTime('yyyy/mm/dd',StrToDate(StrGrid.Cells[5,IntRow])));
            end;
          end else
           MessageBox(Handle,PChar('Helper tidak bisa dirubah'+Chr(13)+Chr(13)+'Sudah ada surat jalan'),'Geser Helper',MB_OK or MB_ICONERROR or MB_SYSTEMMODAL or MB_SETFOREGROUND);
        end;
...
begin
  ...
  if (IntCol<>8) AND ((IntCol<>9)) AND (IntCol<>10) AND (IntCol<>11) then
    Abort
  else begin
    if (IntCol=8) then begin
      ...
      GeserDriver2.Visible :=False;
    end else if (IntCol=9) then begin
      GeserUnit1.Visible := False;
      GeserDriver1.Visible := True;
      GeserHelper.Visible  := False;
      GeserDriver2.Visible :=False;
    end else if (IntCol=10)then begin
      GeserUnit1.Visible := False;
      GeserDriver1.Visible := False;
      GeserDriver2.Visible :=True;
      GeserHelper.Visible  := False;
    end else if (IntCol=11)then begin
      GeserUnit1.Visible := False;
      GeserDriver1.Visible := False;
      GeserDriver2.Visible :=False;
...
begin
  IsOk:=True;
  for IntCount:=2 to StrGrid.RowCount-1 do begin
    if (StrGrid.Cells[22,IntCount]<>'') AND (Package.Checked=True) then
    begin
      IsOk:=False;
      StrEMsg:='Butuh otorisasi!';
      MessageBox(Handle,PChar('Sudah ada yang jalan,'+Chr(13)+Chr(13)+StrEMsg),'Penjadwalan',MB_OK or MB_ICONERROR or MB_SYSTEMMODAL or MB_SETFOREGROUND);
...
procedure TBookingForm.GeserDriver2Click(Sender: TObject);
var
  IsAuth,IsOk:Boolean;
  StrEMsg: string;
  IntCount:Integer;
begin
  ...
  if isOk=True then begin
    ...
    AuthorizedForm.StrMessage:=' Otorisasi Alokasi Seluruh Supir (Geser Unit)';
    if (AuthorizedForm.ShowModal<>1) then IsAuth:=False;
    if IsAuth then EmployeeRDList:=TEmployeeRDList.Create(Self,'Bus',1,0,'Reserved-Create-CODRIVER',FormatDateTime('yyyy/mm/dd',StrToDate(StrGrid.Cells[4,IntRow])),FormatDateTime('yyyy/mm/dd',StrToDate(StrGrid.Cells[5,IntRow])), true);
  end;
end;
