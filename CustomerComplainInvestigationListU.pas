unit CustomerComplainInvestigationListU;
...
type
  TCustomerComplainInvestigationList = class(TForm)
    Selesai: TButton;
...
var
  CustomerComplainInvestigationList:TCustomerComplainInvestigationList;

implementation

uses MainU, SPJFormBusU, CustomerComplainFormU,
  CustomerComplainInvestigationFormU, CustomerComplainCorrection;

{$R *.dfm}

constructor TCustomerComplainInvestigationList.Create(AOwner:TComponent;Form_Request:String='';Customer_Id:String='';Tipe_Data:Integer=0;Is_Closed:Byte=0);
begin
  FormRequest:=Form_Request;
  CustomerId :=QuotedStr(Customer_Id);
  TipeData:=Tipe_Data;
  IsClosed:=Is_Closed;
  Main.WriteLog('Form Open: CustomerComplainInvestigationList='+Form_Request+','+Customer_Id,1);
  Initiation:=True;
...
procedure TCustomerComplainInvestigationList.Init;
begin
  Cari.Text:='';
end;

procedure TCustomerComplainInvestigationList.InitGrid;
var IntCount:Integer;
begin
  MinRowGrid:=0;
  GridCCP.ColCount:=21;
  GridCCP.WordWrap:=False;
  GridCCP.ColWidths[0]:=94;
  ...
  GridCCP.ColWidths[17]:=0;
  GridCCP.ColWidths[18]:=0;
  GridCCP.ColWidths[19]:=0;
  GridCCP.ColWidths[20]:=0;

  GridCCP.SizingHeight:=True;

  GridCCP.Cells[0,0]:='Tanggal Investigasi';
...
procedure TCustomerComplainInvestigationList.RefreshCombo;
var Qry:TADOQuery;
    StrQry:String;
    IntCount:Integer;
begin
...
procedure TCustomerComplainInvestigationList.RefreshData;
...
begin
  ...
  if Main.OpenDb then begin
    ...
    if (Qry.RecordCount>0) then while not(Qry.Eof) do begin
       ...
       if Qry.FieldValues['investigation_date']<>NULL then
        CCPArr[IntCount][0]:=Qry.FieldValues['investigation_date']
...
procedure TCustomerComplainInvestigationList.RefreshGrid;
var IntCount:Integer;
begin
  ...
  for IntCount:=0 to Length(CCPArr)-1 do begin
    ...
    GridCCP.CellStyle[1,IntCount+1].HorizontalAlignment:=taCenter;
    GridCCP.CellStyle[3,IntCount+1].HorizontalAlignment:=taCenter;
...
procedure TCustomerComplainInvestigationList.SelesaiClick(Sender: TObject);
begin
  Close;
end;

procedure TCustomerComplainInvestigationList.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TCustomerComplainInvestigationList.GridCCPSelectCell(Sender: TObject; ACol,
  ARow: Integer; var CanSelect: Boolean);
begin
  IntRow:=ARow;
end;

procedure TCustomerComplainInvestigationList.GridCCPDblClick(Sender: TObject);
var
  StrQry:string;
  Qry:TADOQuery;
begin
  if (IntRow>MinRowGrid) and (GridCCP.Cells[0,IntRow]<>'') then begin
    if FormRequest='' then begin
      if Main.IsFormOpen('CustomerComplainInvestigationForm')=False then CustomerComplainInvestigationForm:=TCustomerComplainInvestigationForm.Create(nil, GridCCP.Cells[2,IntRow], GridCCP.Cells[1,IntRow], 0, '', IsClosed);
    end else begin
      if UpperCase(FormRequest)='MAIN-CHANGE' then  begin
        if (Main.IsFormOpen('CustomerComplainInvestigationForm')=False) then
            CustomerComplainInvestigationForm:=TCustomerComplainInvestigationForm.Create(nil,GridCCP.Cells[2,IntRow],GridCCP.Cells[1,IntRow],2, '', IsClosed);
//        Close;
        ...
        if (Main.IsFormOpen('CustomerComplainInvestigationForm')=True) then
            CustomerComplainInvestigationForm:=TCustomerComplainInvestigationForm.Create(nil,GridCCP.Cells[2,IntRow],GridCCP.Cells[1,IntRow],2, '', IsClosed);
        Close;
      end else if UpperCase(FormRequest)='INVESTIGATION-CLOSED' then  begin
        if (Main.IsFormOpen('CustomerComplainInvestigationForm')=False) then
            CustomerComplainInvestigationForm:=TCustomerComplainInvestigationForm.Create(nil,GridCCP.Cells[2,IntRow],GridCCP.Cells[1,IntRow],3, '', IsClosed);
        Close;
      end else if UpperCase(FormRequest)='CORRECTION-CREATE' then  begin
        if (Main.IsFormOpen('CustomerComplainCorrectionForm')=True) then
        begin
          Qry:=TADOQuery.Create(Self);
          Qry.Connection:=Main.MyConnection;
          if Main.OpenDb then begin
            StrQry:='SELECT * FROM wh_cust_complain_investigation '+
                    'WHERE cust_complain_id='+QuotedStr(GridCCP.Cells[1,IntRow])+' and '+
          ...
          begin
            CustomerComplainCorrectionForm.SetData2(GridCCP.Cells[1,IntRow]);
            Close;
          end else begin
            MessageBox(0,PChar('Komplain Sudah dilakukan Perbaikan Komplain'+Chr(13)),'Customer Complain Investigation List',MB_OK or MB_ICONWARNING);
          end;
        end;
      end
    end;
...
procedure TCustomerComplainInvestigationList.Show;
begin
...
procedure TCustomerComplainInvestigationList.FormShow(Sender: TObject);
begin
  InitGrid;
  RefreshCombo;
  Show;
...
procedure TCustomerComplainInvestigationList.CariChange(Sender: TObject);
...
procedure TCustomerComplainInvestigationList.ToXCelClick(Sender: TObject);
begin
  if ToExcel4(GridCCP) then ShowMessage('Export ke Excel Berhasil')
  else ShowMessage('Export ke Excel Gagal');
end;

procedure TCustomerComplainInvestigationList.AllFowardClick(Sender: TObject);
begin
...
procedure TCustomerComplainInvestigationList.AllClick(Sender: TObject);
begin
  RefreshData;
  RefreshGrid;
end;

procedure TCustomerComplainInvestigationList.FowardClick(Sender: TObject);
begin
  RefreshData;
  RefreshGrid;
end;

procedure TCustomerComplainInvestigationList.BackwardClick(Sender: TObject);
begin
  RefreshData;
  RefreshGrid;
end;

procedure TCustomerComplainInvestigationList.SBUChange(Sender: TObject);
begin
  RefreshData;
  RefreshGrid;
...
procedure TCustomerComplainInvestigationList.LihatClick(Sender: TObject);
begin
  RefreshData;
  RefreshGrid;
end;
