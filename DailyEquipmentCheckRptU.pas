...
type
  TDailyEquipmentCheckRpt = class(TForm)
...
var
  DailyEquipmentCheckRpt: TDailyEquipmentCheckRpt;
...
constructor TDailyEquipmentCheckRpt.Create(AOwner:TComponent);
begin
  FormRequest:='';
  CustId:='';
  CompId:=1;
...
constructor TDailyEquipmentCheckRpt.Create(AOwner:TComponent;Company_Id:String;Form_Request:String='';Customer_Id:String='';Is_All:Integer=6;Update_Time:Boolean=False);
begin
...
procedure TDailyEquipmentCheckRpt.Init;
begin
  Case IsAll of
    8:Caption:='Laporan Penjualan Harian Batal';
    else Caption:='Laporan Penjualan Harian';
  end;
  SBU.Text:='';
  SBU.Items.Clear;
  SBU.ItemIndex:=-1;
...
procedure TDailyEquipmentCheckRpt.InitGrid;
...
procedure TDailyEquipmentCheckRpt.RefreshCombo;
var Qry:TADOQuery;
    StrQry:String;
    IntCount:Integer;
begin
...
procedure TDailyEquipmentCheckRpt.RefreshData;
...
begin
  ...
  if Main.OpenDb then begin
    ...
    StrQry:='EXEC GetCustomerOrderList '+StrCompanyId+StrLocationId+StrTanggal+',@Finish='+IntToStr(IsAll)+StrSales+StrSegment+',@PaymentDetail=1;';
    Main.WriteLog('SQL :'+StrQry,2);
    Qry.SQL.Add(StrQry);
    Qry.Open;
    IntCount:=0;
...
procedure TDailyEquipmentCheckRpt.RefreshGrid;
...
procedure TDailyEquipmentCheckRpt.FormShow(Sender: TObject);
begin
  RefreshDisplay;
  Initiation:=False;
end;

procedure TDailyEquipmentCheckRpt.RefreshDisplay;
begin
  Init;
  InitGrid;
  RefreshCombo;
...
procedure TDailyEquipmentCheckRpt.StrGridSelectCell(Sender: TObject; ACol,
...
procedure TDailyEquipmentCheckRpt.StrGridDblClick(Sender: TObject);
begin
...
procedure TDailyEquipmentCheckRpt.SelesaiClick(Sender: TObject);
begin
  Close;
end;

procedure TDailyEquipmentCheckRpt.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TDailyEquipmentCheckRpt.ToXCelClick(Sender: TObject);
begin
  if ToExcel4(StrGrid) then ShowMessage('Export ke Excel Berhasil')
  else ShowMessage('Export ke Excel Gagal');
end;

procedure TDailyEquipmentCheckRpt.TanggalChange(Sender: TObject);
begin
...
procedure TDailyEquipmentCheckRpt.SalesPersonChange(Sender: TObject);
begin
  if Not(Initiation) then begin
    RefreshData;
    RefreshGrid;
...
procedure TDailyEquipmentCheckRpt.SegmentChange(Sender: TObject);
begin
  RefreshData;
  RefreshGrid;
end;

procedure TDailyEquipmentCheckRpt.SBUChange(Sender: TObject);
begin
  if Not(Initiation) then begin
    RefreshData;
    RefreshGrid;
