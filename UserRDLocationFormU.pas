...
type
  TUserRDLocationForm = class(TForm)
    Batal: TButton;
    Simpan: TButton;
    LabelStatus: TLabel;
    Label5: TLabel;
...
var
  UserRDLocationForm: TUserRDLocationForm;
...
constructor TUserRDLocationForm.Create(AOwner:TComponent;UserLocationId:String='';Form_Request:String='';Is_Input:Boolean=False);
begin
...
procedure TUserRDLocationForm.Init;
begin
...
procedure TUserRDLocationForm.RefreshUser;
var Qry:TADOQuery;
    StrQry:String;
    IntCount:Integer;
begin
...
procedure TUserRDLocationForm.LoadData;
var Qry:TADOQuery;
...
begin
  ...
  if Main.OpenDb then begin
    StrQry:='SELECT dbo.GetUsername(a.user_id) AS name,c.name AS company,* '+
            ...
            'WHERE a.user_rd_location_id='+QuotedStr(StrUserLocationId)+';';
    Qry.SQL.Add(StrQry);
    Main.WriteLog('SQL :'+StrQry,2);
    Qry.Open;
    if Qry.RecordCount>0 then begin
      UserLocId.Text:=Qry.FieldValues['user_rd_location_id'];
...
procedure TUserRDLocationForm.RefreshCompanyLocation;
var Qry:TADOQuery;
...
begin
  ...
  if StrUserLocationId<>'' then StrUserCompanyLocation:='OR (b.user_rd_location_id='+StrUserLocationId+') ' else StrUserCompanyLocation:='';
  if Main.OpenDb then begin
    StrQry:='SELECT * FROM wh_company_location a '+
            'LEFT JOIN wh_user_rd_location b ON (b.location_id=a.company_location_id) AND (b.user_id=dbo.GetUserId('+QuotedStr(Username.Text)+')) '+
            'LEFT JOIN wh_company c ON c.company_id=a.company_id '+
            'LEFT JOIN wh_location d ON d.location_id=a.location_id '+
            'WHERE ((b.user_rd_location_id IS NULL) '+StrUserCompanyLocation+' ) ;';
    Main.WriteLog('SQL :'+StrQry,2);
    Qry.SQL.Add(StrQry);
    Qry.Open;
    if Qry.RecordCount>0 then begin
...
procedure TUserRDLocationForm.EnableInput;
begin
...
procedure TUserRDLocationForm.DisableInput;
begin
...
procedure TUserRDLocationForm.FormShow(Sender: TObject);
begin
  Init;
  InitGrid;
  RefreshUser;
...
procedure TUserRDLocationForm.UsernameChange(Sender: TObject);
begin
  RefreshCompanyLocation;
  InitGrid;
  LoadDataGrid;
...
procedure TUserRDLocationForm.InitGrid;
begin
  StrGrid.RowCount:=2;
...
procedure TUserRDLocationForm.LoadDataGrid;
var Qry:TADOQuery;
...
begin
  ...
  Qry.Connection:=Main.MyConnection;
  if Main.OpenDb then begin
    StrQry:='EXEC GetUserRDLocationList @Name='+QuotedStr(Username.Text)+' ;';
    Main.WriteLog('SQL :'+StrQry,2);
    Qry.SQL.Clear;
    Qry.SQL.Add(StrQry);
    Qry.Open;
    ...
    if Qry.RecordCount>0 then while Not(Qry.Eof) do begin
      UserLocationArr[IntCount][0]:=Qry.FieldValues['user_rd_location_id'];
...
procedure TUserRDLocationForm.RefreshGrid;
...
procedure TUserRDLocationForm.CompanyLocationChange(Sender: TObject);
begin
  InitGrid;
  LoadDataGrid;
  RefreshGrid;
...
procedure TUserRDLocationForm.SimpanClick(Sender: TObject);
var Qry:TADOQuery;
  ...
  if (Trim(CompanyLocation.Text)<>'') and (Trim(Username.Text)<>'')  then begin
    ...
    if Main.OpenDb then begin
      if StrUserLocationId<>'' then
        StrQry:='UPDATE wh_user_rd_location SET company_location_id='+QuotedStr(StrCompanyLocation)+
                ',active='+IntToStr(IntActive)+',update_time=GETDATE(),update_user='+QuotedStr(User)+
                ' WHERE user_rd_location_id='+QuotedStr(StrUserLocationId)+';'
      else
        StrQry:='INSERT INTO wh_user_rd_location (user_id,location_id,update_time,update_user)'+
...
procedure TUserRDLocationForm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TUserRDLocationForm.BatalClick(Sender: TObject);
begin
  Close;
end;

procedure TUserRDLocationForm.ActiveKeyPress(Sender: TObject; var Key: Char);
begin
  if Key=#13 then Simpan.SetFocus;
end;

procedure TUserRDLocationForm.UsernameKeyPress(Sender: TObject; var Key: Char);
begin
  if Key=#13 then CompanyLocation.SetFocus;
end;

procedure TUserRDLocationForm.CompanyLocationKeyPress(Sender: TObject; var Key: Char);
begin
  if Key=#13 then Active.SetFocus;
end;

procedure TUserRDLocationForm.btnClearClick(Sender: TObject);
begin
  Init;
  InitGrid;
  RefreshUser;
