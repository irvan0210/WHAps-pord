...
type
  TIntegrateGPTollTaxi = class(TForm)
...
var
  IntegrateGPTollTaxi: TIntegrateGPTollTaxi;
...
procedure TIntegrateGPTollTaxi.Init;
begin
  FromDates.Date:=Now();
  Lokasi.ItemIndex:=-1;
  LokasiDisp.Text:='';
...
procedure TIntegrateGPTollTaxi.InitGrid1;
...
procedure TIntegrateGPTollTaxi.InitGrid2;
var IntCount,IntCount2:Integer;
begin
  for IntCount:=0 to StrGrid2.RowCount-1 do
    for IntCount2:=0 to StrGrid2.ColCount-1 do
...
procedure TIntegrateGPTollTaxi.RefreshJenis;
var StrQry:String;
    Qry:TADOQuery;
    IntCount:Integer;
begin
...
procedure TIntegrateGPTollTaxi.RefreshCombo;
var QStr:String;
    Qry:TADOQuery;
    Count:Integer;
begin
...
procedure TIntegrateGPTollTaxi.RefreshData1;
...
begin
  ...
  if Main.OpenDb then begin
    ...
    StrQry:='EXEC IntegrateGPTX13 '+QuotedStr(FormatDateTime('yyyy/mm/dd',FromDates.Date))+','+QuotedStr(Lokasi.Text)+',@Total=1'+';';
    Qry.SQL.Add(StrQry);
    Qry.Open;
    ...
    Qry.Close;
    StrQry:='EXEC IntegrateGPTX14 '+QuotedStr(FormatDateTime('yyyy/mm/dd',FromDates.Date))+','+QuotedStr(Lokasi.Text)+';';
    Qry.SQL.Clear;
    Qry.SQL.Add(StrQry);
    Qry.Open;
    IntCount:=0;
...
procedure TIntegrateGPTollTaxi.RefreshData2;
var Qry,Qry2:TADOQuery;
...
begin
  ...
  if Main.OpenDb then begin
    ...
    IntCount:=1;
    StrQry:='EXEC IntegrateGPTX13 '+QuotedStr(FormatDateTime('yyyy/mm/dd',FromDates.Date))+','+QuotedStr(Lokasi.Text)+',@Total=0'+';';    Qry.SQL.Add(StrQry);
    Qry.Open;
    if Qry.RecordCount>0 then begin
      if Qry.FieldValues['total']<>NULL then begin
    ...
    StrLastIndex1:='';
    StrLastIndex2:='';
    StrQry:='EXEC IntegrateGPTX15 '+QuotedStr(FormatDateTime('yyyy/mm/dd',FromDates.Date))+','+QuotedStr(Lokasi.Text)+';';
    Qry.SQL.Clear;
    Qry.SQL.Add(StrQry);
    Qry.Open;
    if Qry.RecordCount>0 then while not(Qry.Eof) do begin
    ...
    Qry.Close;
    Inc(IntCount);
    StrQry:='EXEC IntegrateGPTX16 '+QuotedStr(FormatDateTime('yyyy/mm/dd',FromDates.Date))+','+QuotedStr(Lokasi.Text)+';';
    Qry.SQL.Clear;
    Qry.SQL.Add(StrQry);
    Qry.Open;
    IntCount2:=1;
...
procedure TIntegrateGPTollTaxi.FormShow(Sender: TObject);
begin
  Init;
  RefreshCombo;
  RefreshJenis;
...
procedure TIntegrateGPTollTaxi.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
end;

procedure TIntegrateGPTollTaxi.SelesaiClick(Sender: TObject);
begin
  Close;
end;

procedure TIntegrateGPTollTaxi.ToXCel1Click(Sender: TObject);
begin
  if ToExcel4(StrGrid1) then ShowMessage('Export ke Excel Berhasil')
  else ShowMessage('Export ke Excel Gagal');
end;

procedure TIntegrateGPTollTaxi.ToXcel2Click(Sender: TObject);
begin
...
procedure TIntegrateGPTollTaxi.ProcessClick(Sender: TObject);
begin
  ProcessData;
end;

procedure TIntegrateGPTollTaxi.ProcessData;
begin
  InitGrid1;
  InitGrid2;
  RefreshData1;
...
procedure TIntegrateGPTollTaxi.GPLastGLKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Not(Key In ['0'..'9',#8,#13]) then Key:=#0;
end;

procedure TIntegrateGPTollTaxi.LokasiChange(Sender: TObject);
begin
  LokasiDisp.Text:=LokasiArr[Lokasi.ItemIndex][1];  
end;
